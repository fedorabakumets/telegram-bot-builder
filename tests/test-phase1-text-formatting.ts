...
test('01', 'Обычный текст — базовый случай', () => {
  const code = gen(patchStart({ messageText: 'Привет! Добро пожаловать!', formatMode: 'none' }), 't01');
...
test('04', 'Очень длинный текст (5000 символов)', () => {
  ...
  const code = gen(patchStart({ messageText: longText, formatMode: 'none' }), 't04');
...
test('05', 'Текст с двойными кавычками → экранирование', () => {
  const code = gen(patchStart({ messageText: 'Он сказал: "Привет!"', formatMode: 'none' }), 't05');
...
test('13', 'Текст с переменной {user_name} → replace_variables_in_text', () => {
  const code = gen(patchStart({ messageText: 'Привет, {user_name}!', formatMode: 'none' }), 't13');
...
test('14', 'Текст с несколькими переменными {var1} и {var2}', () => {
  const code = gen(patchStart({ messageText: 'Имя: {user_name}, Возраст: {age}, Город: {city}', formatMode: 'none' }), 't14');
...
test('15', 'formatMode: "none" → нет parse_mode', () => {
  const code = gen(patchStart({ messageText: 'Текст', formatMode: 'none', markdown: false }), 't15');
...
test('16', 'formatMode: "html" → parse_mode="HTML"', () => {
  const code = gen(patchStart({ messageText: '<b>Жирный</b>', formatMode: 'html' }), 't16');
...
test('17', 'formatMode: "html" + все HTML-теги', () => {
  ...
  const code = gen(patchStart({ messageText: html, formatMode: 'html' }), 't17');
...
test('20', 'formatMode: "markdown" → parse_mode="Markdown"', () => {
  const code = gen(patchStart({ messageText: '*Жирный* _курсив_', formatMode: 'markdown' }), 't20');
...
test('21', 'formatMode: "markdown" + все MD-конструкции', () => {
  ...
  const code = gen(patchStart({ messageText: md, formatMode: 'markdown' }), 't21');
...
test('22', 'markdown:true + formatMode:"none" → legacy fallback → parse_mode="Markdown"', () => {
  const code = gen(patchStart({ messageText: '*Жирный*', markdown: true, formatMode: 'none' }), 't22');
...
test('23', 'markdown:false + formatMode:"none" → нет parse_mode', () => {
  const code = gen(patchStart({ messageText: 'Обычный текст', markdown: false, formatMode: 'none' }), 't23');
...
test('24', 'markdown:true + formatMode:"html" → formatMode побеждает → parse_mode="HTML"', () => {
  const code = gen(patchStart({ messageText: '<b>Текст</b>', markdown: true, formatMode: 'html' }), 't24');
...
test('28', 'Два узла с разными formatMode — каждый независим', () => {
  const p = deepClone(BASE_PROJECT);
  p.sheets[0].nodes[0].data.messageText = '<b>HTML</b>';
  p.sheets[0].nodes[0].data.formatMode = 'html';
  p.sheets[0].nodes[1].data.messageText = '*Markdown*';
  p.sheets[0].nodes[1].data.formatMode = 'markdown';
...
test('29', 'Текст с Python-ключевыми словами (if, def, class, import)', () => {
  const code = gen(patchStart({ messageText: 'if def class import lambda return yield', formatMode: 'none' }), 't29');
...
test('31', 'Текст с HTML-инъекцией при formatMode:"none" — не интерпретируется', () => {
  const code = gen(patchStart({ messageText: '<script>alert(1)</script>', formatMode: 'none' }), 't31');
...
test('34', 'messageText только из цифр', () => {
  const code = gen(patchStart({ messageText: '1234567890', formatMode: 'none' }), 't34');
