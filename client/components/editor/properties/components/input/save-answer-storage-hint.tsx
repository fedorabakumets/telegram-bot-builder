...
export function getSaveAnswerStorageHint(inputType: SaveAnswerHintInputType): string {
  ...
  if (inputType === 'any') {
    return (
      ...
      'Скачивание входящих фото на диск — отдельная настройка бота «Сохранять входящие медиа».'
    );
  }
  ...
  if (FILE_ID_MEDIA_TYPES.has(inputType)) {
    ...
    return (
      ...
      `Чтобы сохранять файлы на сервер, включите у бота «Сохранять входящие медиа» (вкладка «Бот»).`
    );
  }
