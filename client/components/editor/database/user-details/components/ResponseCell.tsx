...
export function ResponseCell({ responseData, answerValue }: ResponseCellProps): React.JSX.Element {
  ...
  if (responseData?.media && Array.isArray(responseData.media) && responseData.media.length > 0) {
    return (
      <div className="rounded-lg overflow-hidden max-w-[80px] xs:max-w-[100px] sm:max-w-[120px] space-y-1">
  ...
  // Текст
  return (
    <p className="text-[10px] xs:text-[10px] sm:text-sm text-green-800 dark:text-green-200 font-medium break-words max-w-full">
      {answerValue}
    </p>
  );
}
