/**
 * @fileoverview Утилита для нормализации имени проекта в допустимое имя файла
 *
 * Этот модуль предоставляет функцию для преобразования имени проекта
 * в корректное имя файла, удаляя недопустимые символы.
 */

/**
 * Нормализует имя проекта в допустимое имя файла
 *
...
export function normalizeProjectNameToFile(projectName: string): string {
  ...
  let normalized = projectName
    ...
    .replace(/[^\p{L}\p{N}_\s-]/gu, '')  // удаляем все недопустимые символы
