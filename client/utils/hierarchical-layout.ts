 ...
 */
const ROOT_TYPES = new Set(['start', 'command_trigger', 'text_trigger', 'incoming_message_trigger', 'group_message_trigger', 'callback_trigger', 'incoming_callback_trigger', 'outgoing_message_trigger', 'managed_bot_updated_trigger', 'schedule_trigger']);
  ...
  for (const node of nodesById.values()) {
    ...
    if (typeof data.autoTransitionTo === 'string' && data.autoTransitionTo) {
      pushConnection({
        ...
        type: node.type === 'command_trigger' || node.type === 'text_trigger' || (node.type as any) === 'managed_bot_updated_trigger' || (node.type as any) === 'schedule_trigger' ? 'trigger-next' : 'auto-transition',
      });
    }
