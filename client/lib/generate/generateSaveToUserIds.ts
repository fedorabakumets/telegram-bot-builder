...
export function generateSaveToUserIdsCode(node: Node, indent: string = '        '): string {
  ...
  return [
    ...
    `${indent}            """`,
    `${indent}            INSERT INTO user_ids (user_id, source)`,
    `${indent}            VALUES ($1, $2)`,
    `${indent}            ON CONFLICT (user_id) DO NOTHING`,
    `${indent}            """,`,
    `${indent}            int(user_text),  # Преобразуем введённый текст в число`,
    `${indent}            'bot'  # Источник: бот`,
    `${indent}        )`,
    `${indent}        logging.info(f"✅ ID {user_text} вставлен в таблицу user_ids")`,
