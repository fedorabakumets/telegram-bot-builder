...
export function generateSaveToCsvCode(node: Node, indent: string = '        '): string {
  ...
  return [
    ...
    `${indent}    csv_dir = os.path.join(os.getcwd(), 'bots', f'project_{PROJECT_ID}')`,
    `${indent}    os.makedirs(csv_dir, exist_ok=True)`,
    `${indent}    csv_file = os.path.join(csv_dir, 'user_ids.csv')`,
