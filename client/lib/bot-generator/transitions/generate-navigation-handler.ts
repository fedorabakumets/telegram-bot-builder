...
export function generateNavigationHandler(
  ...
  // Проверка на узел рассылки - не генерируем текст сообщения
  if (navTargetNode.type === 'broadcast') {
    code += `${indent}# Узел рассылки - обработка выполняется отдельно\n`;
    return { code, needsFullHandler, handlerType: 'broadcast' };
  }
