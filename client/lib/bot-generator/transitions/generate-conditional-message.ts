...
export function generateConditionalMessage(
  ...
  if (condition.condition === 'user_data_exists' && variableNames.length > 0) {
    ...
    if (shouldGenerateKeyboard) {
      ...
      code += `${indent}    text = replace_variables_in_text(text, user_data_dict, variable_filters)\n`;
...
function generateConditionalKeyboard(
  ...
  if (condition.keyboardType === 'inline') {
    ...
    code += `${innerIndent}# Получаем фильтры переменных для замены\n`;
    code += `${innerIndent}variable_filters = user_data.get(user_id, {}).get("_variable_filters", {})\n`;
    code += `${innerIndent}text = replace_variables_in_text(text, user_data_dict, variable_filters)\n`;
