      ...
      if (!processedCallbacks.has(nodeId)) {
        ...
        if (targetNode) {
          ...
          if (isBroadcastNode(targetNode.type)) {
            ...
            // Подготовка текста сообщения и переменных (только для НЕ broadcast узлов)
          ...
          if (targetNode.data?.allowMultipleSelection || hasSelectionButtons) {
            ...
            // Пропускаем для broadcast узла - у него нет текста сообщения
            if (targetNode.type === 'broadcast') {
              // Для broadcast узла не генерируем код отправки сообщения
            } else if (targetNode.data.keyboardType === 'reply') {
          ...
          // ИСПРАВЛЕНИЕ: Для broadcast узла не отправляем сообщение - только выполняем рассылку
          const isBroadcastNodeType = targetNode.type === 'broadcast';

          if (!isCommandNode && !isBroadcastNodeType) {
          ...
          if (sourceNode) {
            ...
            if (shouldSaveButtonClick) {
              code += '    # Сохраняем нажатие кнопки в базу данных ТОЛЬКО если это не fake callback (не автопереход)\n';
              code += '    if not is_fake_callback:\n';
              ...
              nodes.forEach((navTargetNode, index) => {
                ...
                if (navTargetNode.type === 'message') {
                  ...
                  if (hasConditionalMessages) {
                    ...
                    // Пропускаем обработку для broadcast узла
                    if (navTargetNode.type === 'broadcast') {
                      code += `                # Узел рассылки - обработка выполняется отдельно\n`;
                    } else if (navTargetNode.data.allowMultipleSelection === true) {
