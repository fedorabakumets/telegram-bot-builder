...
export function message_logging_middleware(codeLines: string[]) {
    ...
    middlewareCodeLines.push('                is_localhost_media = "localhost" in media_api_url or "127.0.0.1" in media_api_url or "0.0.0.0" in media_api_url');
    middlewareCodeLines.push('                is_http_media = media_api_url.startswith("http://")');
    middlewareCodeLines.push('                is_https_media = media_api_url.startswith("https://")');
    middlewareCodeLines.push('                use_ssl_media = is_https_media and not is_localhost_media  # SSL только для внешних https://');
    middlewareCodeLines.push('                logging.debug(f"🔒 SSL требуется для медиа-запроса {media_api_url}: {use_ssl_media}")');
    middlewareCodeLines.push('                ');
    middlewareCodeLines.push('                # Создаём connector с правильным SSL');
    middlewareCodeLines.push('                connector = TCPConnector(ssl=use_ssl_media)');
    middlewareCodeLines.push('                ');
    middlewareCodeLines.push('                async with aiohttp.ClientSession(connector=connector) as session:');
    middlewareCodeLines.push('                    async with session.post(media_api_url, json=media_payload, timeout=aiohttp.ClientTimeout(total=API_TIMEOUT), ssl=use_ssl_media) as response:');
