...
export function replace_variables_in_text(codeLines: string[], indentLevel: string = '') {
  ...
  replaceCodeLines.push(`${indentLevel}def replace_variables_in_text(text_content, variables_dict, variable_filters=None):`);
  ...
  replaceCodeLines.push(`${indentLevel}        variable_filters (dict, optional): Словарь фильтров для переменных`);
  replaceCodeLines.push(`${indentLevel}    `);
  ...
  replaceCodeLines.push(`${indentLevel}    `);
  replaceCodeLines.push(`${indentLevel}    # Инициализируем фильтры если не переданы`);
  replaceCodeLines.push(`${indentLevel}    if variable_filters is None:`);
  replaceCodeLines.push(`${indentLevel}        variable_filters = {}`);
  replaceCodeLines.push(`${indentLevel}    `);
  ...
  replaceCodeLines.push(`${indentLevel}            elif isinstance(var_data, list):`);
  replaceCodeLines.push(`${indentLevel}                # Если переменная - массив (appendVariable), применяем фильтр`);
  replaceCodeLines.push(`${indentLevel}                filter_rule = variable_filters.get(var_name, "")`);
  replaceCodeLines.push(`${indentLevel}                # Проверяем фильтр на разделитель`);
  replaceCodeLines.push(`${indentLevel}                import re`);
  replaceCodeLines.push(`${indentLevel}                match = re.search(r'\\|join:"([^"]+)"', filter_rule)`);
  replaceCodeLines.push(`${indentLevel}                if match:`);
  replaceCodeLines.push(`${indentLevel}                    separator = match.group(1)`);
  replaceCodeLines.push(`${indentLevel}                    # Преобразуем escape-последовательности`);
  replaceCodeLines.push(`${indentLevel}                    separator = separator.replace("\\\\n", "\\n").replace("\\n", "\\n")`);
  replaceCodeLines.push(`${indentLevel}                    var_value = separator.join(str(item) for item in var_data)`);
  replaceCodeLines.push(`${indentLevel}                else:`);
  replaceCodeLines.push(`${indentLevel}                    # По умолчанию запятая`);
  replaceCodeLines.push(`${indentLevel}                    var_value = ", ".join(str(item) for item in var_data)`);
