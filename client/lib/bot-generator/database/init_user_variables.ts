...
export function init_user_variables(codeLines: string[], indentLevel: string = '') {
  ...
  initUserCodeLines.push(`${indentLevel}async def init_user_variables(user_id, user_obj):`);
  ...
  initUserCodeLines.push(`${indentLevel}    `);
  initUserCodeLines.push(`${indentLevel}    # ЗАГРУЖАЕМ данные из базы данных для восстановления переменных после перезапуска`);
  initUserCodeLines.push(`${indentLevel}    # Примечание: вызывается из async функции, поэтому используем await`);
  initUserCodeLines.push(`${indentLevel}    db_user_vars = await get_user_from_db(user_id)`);
  initUserCodeLines.push(`${indentLevel}    if db_user_vars and isinstance(db_user_vars, dict):`);
  initUserCodeLines.push(`${indentLevel}        # Сохраняем все переменные из БД в локальное хранилище`);
  initUserCodeLines.push(`${indentLevel}        for key, value in db_user_vars.items():`);
  initUserCodeLines.push(`${indentLevel}            if key not in ["user_id", "username", "first_name", "last_name", "user_data", "registered_at", "last_interaction"]:`);
  initUserCodeLines.push(`${indentLevel}                # Если значение строка (старые данные), преобразуем в массив для совместимости`);
  initUserCodeLines.push(`${indentLevel}                if isinstance(value, str):`);
  initUserCodeLines.push(`${indentLevel}                    user_data[user_id][key] = [value]`);
  initUserCodeLines.push(`${indentLevel}                else:`);
  initUserCodeLines.push(`${indentLevel}                    user_data[user_id][key] = value`);
  initUserCodeLines.push(`${indentLevel}        logging.info(f"✅ Загружено {len(db_user_vars)} переменных из БД для пользователя {user_id}")`);
  initUserCodeLines.push(`${indentLevel}    `);
