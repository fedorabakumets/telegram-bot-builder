...
export function save_user_to_db(codeLines: string[]) {
    ...
    saveUserCodeLines.push('async def save_user_to_db(user_id: int, username: Optional[str] = None, first_name: Optional[str] = None, last_name: Optional[str] = None, avatar_url: Optional[str] = None):');
    ...
    saveUserCodeLines.push('                INSERT INTO bot_users (user_id, username, first_name, last_name, avatar_url)');
    saveUserCodeLines.push('                VALUES ($1, $2, $3, $4, $5)');
    ...
    saveUserCodeLines.push('                    avatar_url = COALESCE(EXCLUDED.avatar_url, bot_users.avatar_url),');
    saveUserCodeLines.push('                    last_interaction = NOW(),');
    saveUserCodeLines.push('                    interaction_count = bot_users.interaction_count + 1');
    saveUserCodeLines.push('            """, user_id, username, first_name, last_name, avatar_url)');
