...
/**
 * Генерирует Python-код для обработки условных сообщений в start узле
 ...
 */
export function generateStartNodeConditionalMessageCode(
  ...
  if (conditionalMessages && conditionalMessages.length > 0) {
    code += `${indent}\n`;
    code += `${indent}# Проверка условных сообщений для start узла\n`;
