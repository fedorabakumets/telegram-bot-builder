...
export function generateStartHandler(node: Node, userDatabaseEnabled: boolean): string {
  ...
  if (attachedMedia.length > 0) {
    ...
    mediaCode += '                # Проверяем, является ли API_BASE_URL локальным адресом\n';
    ...
    mediaCode += '                    # Временно используем публичный URL, если он доступен, иначе отправляем текстовое сообщение\n';
