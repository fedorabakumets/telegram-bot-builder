// Внешние зависимости
import { z } from 'zod';
import { BotData, BotGroup, buttonSchema } from '@shared/schema';
...
export function generatePythonCode(botData: BotData, botName: string = "MyBot", groups: BotGroup[] = [], userDatabaseEnabled: boolean = false, projectId: number | null = null, enableLogging: boolean = false): string {
  ...
  code = generateGroupBasedEventHandlers(groups, code);
  ...
  code = generateFallbackHandlers(userDatabaseEnabled, code);

  code = generateMainFunctionScaffoldWithSignalHandlers(userDatabaseEnabled, code);
  code = generateBotInitializationAndMiddlewareSetup(userDatabaseEnabled, menuCommands, nodes, hasInlineButtons, code);
  code = generateMainPollingLoopWithGracefulShutdown(userDatabaseEnabled, code);
  ...
  code = generateMultiSelectCallbackDispatcherHandle(multiSelectNodes, code, generateMultiSelectDataPersistenceAndCleanupCode, generateTransitionLogicForMultiSelectCompletion);
  ...
  function generateInteractiveCallbackHandlersWithConditionalMessagesMultiSelectAndAutoNavigation(): void {
    if (inlineNodes.length > 0 || allReferencedNodeIds.size > 0 || allConditionalButtons.size > 0) {
      ...
      processNodeButtonsAndGenerateHandlers(inlineNodes, processedCallbacks, nodes, code, allNodeIds, connections, mediaVariablesMap);
          ...
          if (targetNode) {
            ...
            if (targetNode.data.allowMultipleSelection || hasSelectionButtons) {
              ...
              if (multiSelectKeyboardType === 'reply') {
                ...
                if (selectionButtons.length > 0) {
                  if (isLoggingEnabled()) isLoggingEnabled() && console.log(`🔧 ГЕНЕРАТОР: ✅ ДО��АВЛЯЕМ кнопку "Готово" (есть ${selectionButtons.length} кнопок выбора)`);
                      ...
                      for (let i = 0; i < conditionalMessages.length; i++) {
                        ...
                        if (condition.condition === 'user_data_exists' && variableNames.length > 0) {
                          ...
                          if (shouldGenerateKeyboard) {
                            code += '                # Создаем ��������������������������лавиатуру для условного сообщения\n';
