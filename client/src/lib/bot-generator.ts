 ...
 */
function generateButtonResponseHandlers(code: string, userInputNodes: Node[], nodes: Node[]): string {
  /**
   * Генерирует код для обработчика кнопочного ответа
   * @param {string} code - код, в который будет добавляться новый обработчик
   * @param {Node[]} userInputNodes - массив узлов, для которых нужно добавить обработчики
   * @param {Node[]} nodes - массив узлов, которые могут быть следующими в навигации
   * @returns {string} - обновленный код
   */
