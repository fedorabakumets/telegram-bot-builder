...
import { Button, globalLoggingEnabled, isLoggingEnabled, logFlowAnalysis } from './bot-generator';
import { generateBotFatherCommands } from './commands';
import { collectConditionalMessageButtons, generateConditionalButtonHandlerCode, hasConditionalValueButtons } from './Conditional';
import { generateDatabaseCode } from './database';
...
import { generateBasicBotSetupCode } from './generate/generateBasicBotSetupCode';
import { generateGroupsConfiguration } from './generate/generateGroupsConfiguration';
import { generateNodeNavigation } from './generate/generateNodeNavigation';
import { generateSafeEditOrSendCode } from './generate/generateSafeEditOrSendCode';
import { generateUtf8EncodingCode } from './generate/generateUtf8EncodingCode';
...
import { filterInlineNodes, generateButtonResponseHandlers, generateInlineKeyboardCode, generateReplyButtonHandlers } from './Keyboard';
...
import { generateMessageLoggingCode } from './logging';
import { extractNodesAndConnections, generateMediaFileFunctions, hasMediaNodes, hasUploadImageUrls } from './MediaHandler';
...
export function generatePythonCode(botData: BotData, botName: string = "MyBot", groups: BotGroup[] = [], userDatabaseEnabled: boolean = false, projectId: number | null = null, enableLogging: boolean = false, enableGroupHandlers: boolean = false): string {
    ...
    const hasUrlImageNodes = (nodes || []).some(node => node.data?.imageUrl && node.data.imageUrl.startsWith('http')
    );
    ...
    const hasNodesRequiringDatetime = (nodes || []).some(node => node.type === 'command' || // Команды могут использовать datetime для логирования времени вызова
    ...
    const hasNodesRequiringTimezone = (nodes || []).some(node => node.type === 'photo' || // Обработчик фото использует datetime.now(timezone.utc)
    ...
    const hasNodesRequiringParseMode = (nodes || []).some(node => node.data?.formatMode &&
    ...
    const hasNodesRequiringTelegramBadRequest = (nodes || []).some(node => node.type === 'delete_message' ||
    ...
    const menuCommands = (nodes || []).filter(node => (node.type === 'start' || node.type === 'command') &&
