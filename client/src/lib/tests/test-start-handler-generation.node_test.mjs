...
import { generateNodeHandlers } from '../generate/generate-node-handlers.js';
...
try {
  ...
  const codeMultiple = generateNodeHandlers(multipleNodes, false, false); // enableComments: false
  
  // Проверяем, что в коде присутствует определение start_handler
  assert.ok(
    codeMultiple.includes('@dp.message(CommandStart())'),
    'Код должен содержать декоратор @dp.message(CommandStart())'
  );
  
  assert.ok(
    codeMultiple.includes('async def start_handler'),
