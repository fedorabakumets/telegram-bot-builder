...
| `generateBotInitializationAndMiddlewareSetup.ts` | ⚙️ Инициализация бота и настройка middleware | Подключение базы данных |
| `generateFallbackHandlers.ts` | 🛡 Обработчики fallback для необработанных сообщений | Защита от необработанных сообщений |
| `generateGroupBasedEventHandlers.ts` | 🏢 Обработчики событий для групп бота | Поддержка групповых чатов |
| `generateMainFunctionScaffoldWithSignalHandlers.ts` | 🏗 Каркас основной функции с обработчиками сигналов | Корректное завершение работы |
| `generateMainPollingLoopWithGracefulShutdown.ts` | 🔄 Основной цикл опроса с корректным завершением | Обработка сигналов завершения |
| `generateMultiSelectCallbackDispatcherHandle.ts` | 📝 Обработчик callback-запросов для множественного выбора | Поддержка выбора нескольких опций |
| `generateMultiSelectDataPersistenceAndCleanupCode.ts` | 💾 Код сохранения данных множественного выбора и очистки состояния | Управление временными данными |
| `generateTransitionLogicForMultiSelectCompletion.ts` | 🔄 Логика переходов для завершения множественного выбора | Переходы между узлами |
| `identifyNodesRequiringMultiSelectLogic.ts` | 🔍 Идентификация узлов, требующих логику множественного выбора | Автоматическое определение |
| `newprocessNodeButtonsAndGenerateHandlers.ts` | 🎯 Обработка узлов и генерация обработчиков | Обработка кнопок |
| `processNodeButtonsAndGenerateHandlers.ts` | 🎲 Обработка кнопок узлов и генерация обработчиков | Обработка действий |
| `audio-handler.ts` | 🎵 Обработка аудио | Поддержка аудиофайлов |
| `conditional-button-handler.ts` | 🧩 Обработка условных кнопок | Динамические кнопки |
| `document-handler.ts` | 📄 Обработка документов | Поддержка файлов |
| `generate-reply-button-handlers.ts` | 📤 Генерация обработчиков для кнопочных ответов | Обработка ответов |
| `generateCompleteBotScriptFromNodeGraph.ts` | 📑 Генерация полного скрипта бота из графа узлов | Полная генерация |
| `generateGroupHandlers.ts` | 👥 Генерация обработчиков групп | Поддержка групп |
| `generateMediaFileFunctions.ts` | 📷 Генерация функций для работы с медиафайлами | Обработка медиа |
| `generateMultiSelectCallbackLogic.ts` | 🔄 Генерация логики callback для множественного выбора | Callback-логика |
| `generateMultiSelectDoneHandler.ts` | ✅ Генерация обработчика кнопки "Готово" для множественного выбора | Завершение выбора |
| `generateMultiSelectReplyHandler.ts` | 📥 Генерация обработчика ответов для множественного выбора | Обработка ответов |
| `photo-handler.ts` | 📸 Обработка фото | Поддержка изображений |
| `video-handler.ts` | 🎬 Обработка видео | Поддержка видеоконтента |
| `extractNodeData.ts` | 📊 Извлечение данных узлов | Извлечение информации |
| `filterInlineNodes.ts` | 🔍 Фильтрация инлайн-узлов | Фильтрация узлов |
