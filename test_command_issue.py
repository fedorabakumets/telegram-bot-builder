"""
Тест проблемы с командами, которые содержат инлайн кнопки
"""
import json

def create_test_bot():
    """Создаёт тестовый бот для проверки проблемы с командами и инлайн кнопками"""
    
    bot_data = {
        "name": "Тест команд с инлайн кнопками",
        "description": "Тестирует проблему с командами, которые содержат инлайн кнопки",
        "nodes": [
            # Стартовая команда
            {
                "id": "start-node",
            ...
            # Команда помощи с инлайн кнопками
            {
                "id": "help-cmd",
                "type": "command",
        ...
        "connections": [
            ...
            {"from": "help-cmd", "to": "commands-msg"},
            {"from": "help-cmd", "to": "start-node"},
            {"from": "menu-msg", "to": "info-msg"},
            {"from": "menu-msg", "to": "settings-msg"},
            {"from": "menu-msg", "to": "start-node"},
            {"from": "commands-msg", "to": "test-msg"},
            {"from": "commands-msg", "to": "help-cmd"},
            {"from": "test-msg", "to": "commands-msg"},
            {"from": "info-msg", "to": "menu-msg"},
            {"from": "settings-msg", "to": "menu-msg"}
        ]
    }
    
    # Сохраняем в файл для тестирования
    with open('test_command_issue_bot.json', 'w', encoding='utf-8') as f:
...
def main():
    ...
    print("🔧 Создание тестового бота для проверки проблемы с командами...")
    
    bot_data = create_test_bot()
    
    print(f"✅ Создан тестовый бот: {bot_data['name']}")
    print(f"📊 Узлов: {len(bot_data['nodes'])}")
    print(f"🔗 Связей: {len(bot_data['connections'])}")
    print("\n🎯 Проблема для тестирования:")
    print("Команды /start и /help имеют инлайн кнопки и синонимы.")
    print("Проверьте, работают ли синонимы 'старт', 'помощь', 'справка', 'команды'")
    print("когда пользователь пишет их как обычные сообщения.")
    
    # Анализ синонимов
    print("\n📝 Анализ синонимов в боте:")
    for node in bot_data['nodes']:
        if 'synonyms' in node['data'] and node['data']['synonyms']:
            node_type = node['type']
            command = node['data'].get('command', 'N/A')
            has_inline = node['data'].get('keyboardType') == 'inline'
            synonyms = node['data']['synonyms']
            
            print(f"• {node_type} {command}: {synonyms} (инлайн кнопки: {'да' if has_inline else 'нет'})")
