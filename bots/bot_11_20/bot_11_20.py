...
PROJECT_ID = int(os.getenv("PROJECT_ID", 11))  # ID проекта в системе
...
# Код сгенерирован в generateConditionalKeyboard.ts
        ...
        conditional_keyboard = keyboard        # ВАЖНО: Логируем состояние условной клавиатуры для отладки
...
async def handle_callback_f90r9k3FSLu2Tjn74cBn_(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        if next_node_id == "start":
            ...
            keyboard = builder.as_markup(resize_keyboard=True, one_time_keyboard=False)
            # Заменяем все переменные в тексте
            nav_text = replace_variables_in_text(nav_text, user_vars)
            ...
            elif user_id in user_data and user_data[user_id].get("collectUserInput_vxPv7G4n0QGyhnv4ucOM5", False) == True:
...
"""
            ...
            # Проверяем, разрешён ли автопереход для этого узла (collectUserInput)
            elif user_id in user_data and user_data[user_id].get("collectUserInput_8xSJaWAJNz7Hz_54mjFTF", False) == True:
...
4. Изменить текст анкеты."""
            ...
            logging.warning(f"Неизяяестны�� следующий узел: {next_node_id}")
    except Exception as e:
...
# Код сгенерирован в generateAttachedMediaSendCode.ts
    if attached_media and str(attached_media).strip():
        ...
        try:
            ...
            await bot.send_photo(callback_query.from_user.id, attached_media, caption=processed_caption, reply_markup=keyboard, node_id="8xSJaWAJNz7Hz_54mjFTF")
        except Exception as e:
...
# Код сгенерирован в generateAttachedMediaSendCode.ts
        if False:
            ...
            await safe_edit_or_send(callback_query, processed_text, node_id="8xSJaWAJNz7Hz_54mjFTF", reply_markup=keyboard if keyboard is not None else None)    # АВТОПЕРЕХОД: Проверяем, есть ли автопереход для этого узла
    # ИСПРАВЛЕНИЕ: НЕ делаем автопереход если была показана условная клавиатура
    user_id = callback_query.from_user.id
    has_conditional_keyboard = user_data.get(user_id, {}).get("_has_conditional_keyboard", False)
    if has_conditional_keyboard:
...
async def handle_reply_iIkbMb2jlZRJOxGHMNl1a(message: types.Message):
    ...
    logging.info(f"✅ Состояние ожидания настроено: type='{primaryInputType}', modes=['button', 'text'] для переменной sex (узел RFTgm4KzC6dI39AMTPcmo)")
...
@dp.message(lambda message: message.text == "1")
async def handle_reply_kfzexViyPfMpgOffuWXY3(message: types.Message):
...
async def handle_reply_En0QBjOLWkcEpIGLqy6EQ(message: types.Message):
    ...
    logging.info(f"✅ Состояние ожидания настроено: type='{primaryInputType}', modes=['button', 'text'] для переменной info (узел lBPy3gcGVLla0NGdSYb35)")
    await message.answer(text, reply_markup=keyboard)
                    ...
                    while next_node_id:
                        ...
                        if current_node_id == "start":
                            ...
                            if (
                                ...
                                main_text = text
                                # Заменяем все переменные в тексте
                                main_text = replace_variables_in_text(main_text, user_data_dict)
                                await message.answer(main_text, reply_markup=keyboard)
