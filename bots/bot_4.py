...
async def start_handler(message: types.Message):
    ...
    if use_conditional_keyboard:
        ...
        builder.add(InlineKeyboardButton(text="Новая кнопка", callback_data="age_input_copy_1754771009367_paste_1754771025811_njk94zcv5_paste_1754771144161_idbrfletb"))
        builder.add(InlineKeyboardButton(text="Новая кнопка", callback_data="age_input_copy_1754771009367_paste_1754771025811_njk94zcv5_paste_1754771144161_idbrfletb"))
        builder.adjust(1)
...
@dp.callback_query(lambda c: c.data == "age_input_copy_1754771009367_paste_1754771025811_njk94zcv5_paste_1754771144161_idbrfletb" or c.data.startswith("age_input_copy_1754771009367_paste_1754771025811_njk94zcv5_paste_1754771144161_idbrfletb_btn_"))
async def handle_callback_age_input_copy_1754771009367_paste_1754771025811_njk94zcv5_paste_1754771144161_idbrfletb(callback_query: types.CallbackQuery):
...
async def handle_user_input(message: types.Message):
        ...
        if next_node_id:
            try:
                ...
                if next_node_id == "profile_command":
                    ...
                    elif next_node_id == "PT6SZNCzWbgDhkglkYpBb":
                        await handle_callback_PT6SZNCzWbgDhkglkYpBb(fake_callback)
                    else:
        ...
        if selected_option:
            ...
            if option_action == "url" and option_url:
                ...
                try:
                    ...
                    elif target_node_id == "PT6SZNCzWbgDhkglkYpBb":
                        await handle_callback_PT6SZNCzWbgDhkglkYpBb(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=target_node_id, message=message))
                    else:
                ...
                if next_node_id:
                    try:
                        ...
                        if next_node_id == "start":
                            ...
                            await handle_callback_PT6SZNCzWbgDhkglkYpBb(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=next_node_id, message=message))
                        else:
                            logging.warning(f"Неизвестный следующий узел: {next_node_id}")
                    except Exception as e:
                        logging.error(f"Ошибка при переходе к следующему узлу {next_node_id}: {e}")
    ...
    if user_id in user_data and "waiting_for_input" in user_data[user_id]:
            ...
            if next_node_id:
                try:
                    ...
                    if next_node_id == "start":
                        ...
                        text = "7713154819:AAEE4vOE0cpuLpsJZTQPCiAwKMjifDlpnTk"
        ...
        if waiting_node_id == "PT6SZNCzWbgDhkglkYpBb":
            ...
            # Сохраняем в пользовательские данные
            user_data[user_id]["вывы"] = response_data
            
            # Сохраняем в базу данных
            saved_to_db = await update_user_data_in_db(user_id, "вывы", response_data)
            if saved_to_db:
                logging.info(f"✅ Данные сохранены в БД: вывы = {user_text} (пользователь {user_id})")
            else:
                logging.warning(f"⚠️ Не удалось сохранить в БД, данные сохранены локально")
            ...
            logging.info(f"Получен пользовательский ввод: вывы = {user_text}")
