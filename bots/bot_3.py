...
async def start_handler(message: types.Message):
    ...
    art_selected = check_interest_match("Мэн", saved_interests)
    art_text = "✅ Мэн" if art_selected else "Мэн"
    builder.add(InlineKeyboardButton(text=art_text, callback_data="multi_select_start_art"))
    
    games_selected = check_interest_match("вумен", saved_interests)
    games_text = "✅ вумен" if games_selected else "вумен"
...
@dp.callback_query(lambda c: c.data == "age_input_copy_1754771009367_paste_1754771025811_njk94zcv5" or c.data.startswith("age_input_copy_1754771009367_paste_1754771025811_njk94zcv5_btn_"))
async def handle_callback_age_input_copy_1754771009367_paste_1754771025811_njk94zcv5(callback_query: types.CallbackQuery):
    await callback_query.answer()
    user_id = callback_query.from_user.id
    ...
    # Обрабатываем узел age_input_copy_1754771009367_paste_1754771025811_njk94zcv5: age_input_copy_1754771009367_paste_1754771025811_njk94zcv5
...
Напиши свой возраст числом (например, 25):"""
    ...
    button_display_text = "Кнопка age_input_copy_1754771009367_paste_1754771025811_njk94zcv5"
    ...
    next_node_id = "age_input_copy_1754771009367_paste_1754771025811_njk94zcv5"
    try:
...
Теперь вы будете получать персонализированные рекомендации на основе ваших интересов."""
        ...
        elif next_node_id == "age_input_copy_1754771009367_paste_1754771025811_njk94zcv5":
...
Напиши свой возраст числом (например, 25):"""
            ...
            user_data[user_id]["waiting_for_input"] = {
                ...
                "variable": "user_age",
                "save_to_database": True,
                "node_id": "age_input_copy_1754771009367_paste_1754771025811_njk94zcv5",
...
Напиши свой возраст числом (например, 25):"""
    ...
    user_data[callback_query.from_user.id]["waiting_for_input"] = "age_input_copy_1754771009367_paste_1754771025811_njk94zcv5"
            ...
            try:
                ...
                if next_node_id == "profile_command":
                    ...
                    if next_node_id == "start":
                        ...
                        await handle_callback_age_input_copy_1754771009367_paste_1754771025811_njk94zcv5(fake_callback)
                    else:
                        logging.warning(f"Неизвестный следующий узел: {next_node_id}")
            except Exception as e:
                logging.error(f"Ошибка при переходе к следующему узлу {next_node_id}: {e}")
        ...
        if selected_option:
            ...
            if option_action == "url" and option_url:
                ...
                try:
                    ...
                    elif target_node_id == "age_input_copy_1754771009367_paste_1754771025811_njk94zcv5":
                        await handle_callback_age_input_copy_1754771009367_paste_1754771025811_njk94zcv5(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=target_node_id, message=message))
                    else:
                ...
                if next_node_id:
                    try:
                        ...
                        if next_node_id == "start":
                            ...
                            await handle_callback_age_input_copy_1754771009367_paste_1754771025811_njk94zcv5(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=next_node_id, message=message))
                        else:
                            logging.warning(f"Неизвестный следующий узел: {next_node_id}")
                    except Exception as e:
                        logging.error(f"Ошибка при переходе к следующему узлу {next_node_id}: {e}")
...
Напиши свой возраст числом (например, 25):"""
        ...
        if waiting_node_id == "final_message":
            ...
            # Сохраняем в пользовательские данные
            user_data[user_id]["ыапиптктек"] = response_data
            
            # Сохраняем в базу данных
            saved_to_db = await update_user_data_in_db(user_id, "ыапиптктек", response_data)
            if saved_to_db:
                logging.info(f"✅ Данные сохранены в БД: ыапиптктек = {user_text} (пользователь {user_id})")
            else:
                logging.warning(f"⚠️ Не удалось сохранить в БД, данные сохранены локально")
            ...
            logging.info(f"Получен пользовательский ввод: ыапиптктек = {user_text}")
            
            # Переходим к следующему узлу
            try:
                # Отправляем сообщение для узла age_input_copy_1754771009367_paste_1754771025811_njk94zcv5
                text = """Сколько тебе лет? 🎂

Напиши свой возраст числом (например, 25):"""
                # Настраиваем новое ожидание ввода для узла age_input_copy_1754771009367_paste_1754771025811_njk94zcv5
                user_data[user_id]["waiting_for_input"] = {
                    "type": "text",
                    "variable": "user_age",
                    "save_to_database": True,
            ...
            return
        elif waiting_node_id == "age_input_copy_1754771009367_paste_1754771025811_njk94zcv5":
            ...
            # Переходим к следующему узлу
            try:
                logging.warning(f"Целевой узел {node.data.inputTargetNodeId} не найден")
                await message.answer("❌ Ошибка перехода: целевой узел не найден")
            except Exception as e:
                logging.error(f"Ошибка при переходе к следующему узлу: {e}")
            return
    ...
    if len(parts) >= 3:
        ...
        if button_text:
            ...
            if node_id == "start":
                ...
                art_selected = any("Мэн" in interest or "art" in interest.lower() for interest in selected_list)
                art_text = "✅ Мэн" if art_selected else "Мэн"
                builder.add(InlineKeyboardButton(text=art_text, callback_data="multi_select_start_art"))
                # Проверяем каждый интерес и добавляем галочку если он выбран
                games_selected = any("вумен" in interest or "games" in interest.lower() for interest in selected_list)
                games_text = "✅ вумен" if games_selected else "вумен"
                builder.add(InlineKeyboardButton(text=games_text, callback_data="multi_select_start_games"))
                builder.add(InlineKeyboardButton(text="Готово", callback_data="multi_select_done_start"))
    ...
    if user_id in user_data and "multi_select_node" in user_data[user_id] and user_data[user_id].get("multi_select_type") == "reply":
        ...
        if node_id == "start":
            if user_input == "Мэн":
                ...
                if "Мэн" in selected_list:
                    selected_list.remove("Мэн")
                    await message.answer("❌ Убрано: Мэн")
                else:
                    selected_list.append("Мэн")
                    await message.answer("✅ Выбрано: Мэн")
                return
            ...
            if user_input == "вумен":
                ...
                selected_list = user_data[user_id]["multi_select_{node_id}"]
                if "вумен" in selected_list:
                    selected_list.remove("вумен")
                    await message.answer("❌ Убрано: вумен")
                else:
                    selected_list.append("вумен")
                    await message.answer("✅ Выбрано: вумен")
                return
