...
async def start_handler(message: types.Message):
    ...
    builder.add(InlineKeyboardButton(text="ОТ саши", callback_data="MqVWkZdb-tUroJNGs8KlV"))
    builder.add(InlineKeyboardButton(text="От ксюши", callback_data="MqVWkZdb-tUroJNGs8KlV"))
    ...
    user_data[message.from_user.id]["input_node_id"] = "CpQmikCKa310nCCr8kuPX"
    user_data[message.from_user.id]["input_variable"] = "источник3"

# Обработчики inline кнопок

@dp.callback_query(lambda c: c.data == "MqVWkZdb-tUroJNGs8KlV")
async def handle_callback_MqVWkZdb_tUroJNGs8KlV(callback_query: types.CallbackQuery):
...
async def handle_user_input(message: types.Message):
        ...
        if selected_option:
            ...
            if option_action == "url" and option_url:
                ...
                try:
                    ...
                    if target_node_id == "CpQmikCKa310nCCr8kuPX":
                        await handle_callback_CpQmikCKa310nCCr8kuPX(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=target_node_id, message=message))
                    elif target_node_id == "MqVWkZdb-tUroJNGs8KlV":
                        await handle_callback_MqVWkZdb_tUroJNGs8KlV(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=target_node_id, message=message))
                    else:
                ...
                if next_node_id:
                    try:
                        ...
                        if next_node_id == "CpQmikCKa310nCCr8kuPX":
                            await handle_callback_CpQmikCKa310nCCr8kuPX(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=next_node_id, message=message))
                        elif next_node_id == "MqVWkZdb-tUroJNGs8KlV":
                            await handle_callback_MqVWkZdb_tUroJNGs8KlV(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=next_node_id, message=message))
                        else:
    ...
    if user_id in user_data and "waiting_for_input" in user_data[user_id]:
        ...
        if waiting_node_id == "CpQmikCKa310nCCr8kuPX":
            ...
            response_data = {
                ...
                "type": "text",
                "timestamp": timestamp,
                "nodeId": "CpQmikCKa310nCCr8kuPX",
                "variable": "источник3"
            }
            
            # Сохраняем в пользовательские данные
            user_data[user_id]["источник3"] = response_data
            ...
            logging.info(f"Получен пользовательский ввод: источник3 = {user_text}")
    ...
    if next_node_id:
        try:
            ...
            if next_node_id == "CpQmikCKa310nCCr8kuPX":
                logging.info(f"Переход к узлу CpQmikCKa310nCCr8kuPX типа start")
            elif next_node_id == "MqVWkZdb-tUroJNGs8KlV":
                text = "Новое сообщение"
                parse_mode = None
