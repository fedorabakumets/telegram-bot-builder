...
👋 Имя: {имя}"""
    ...
    if use_conditional_keyboard:
        ...
        builder.add(InlineKeyboardButton(text="пол", callback_data="nr3wIiTfBYYmpkkXMNH7n"))
...
@dp.callback_query(lambda c: c.data == "btn-edit-name-default")
async def handle_callback_btn_edit_name_default(callback_query: types.CallbackQuery):
    await callback_query.answer()
    user_id = callback_query.from_user.id
...
@dp.callback_query(lambda c: c.data == "A92WFZ_vXTM1ABx186ldh")
async def handle_callback_A92WFZ_vXTM1ABx186ldh(callback_query: types.CallbackQuery):
    await callback_query.answer()
    user_id = callback_query.from_user.id
    button_text = "пол"
