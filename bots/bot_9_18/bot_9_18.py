...
PROJECT_ID = int(os.getenv("PROJECT_ID", 9))  # ID проекта в системе
...
# @@NODE_START:AoHjXk0_Tz4gVuau5zj-m@@
...
async def handle_callback_AoHjXk0_Tz4gVuau5zj_m(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        logging.info(f"🔵 Вызван callback handler: handle_callback_AoHjXk0_Tz4gVuau5zj_m для пользователя {user_id}")
    except Exception as e:
        logging.error(f"❌ Ошибка доступа к callback_query в handle_callback_AoHjXk0_Tz4gVuau5zj_m: {e}")
        return
...
async def handle_callback_eWKsWq0y8Xlm39S9JEf1g(callback_query: types.CallbackQuery):
    ...
    try:
        if keyboard is not None:
            await bot.send_photo(callback_query.from_user.id, image_url, caption=text, reply_markup=keyboard, node_id="eWKsWq0y8Xlm39S9JEf1g")
        else:
            await bot.send_photo(callback_query.from_user.id, image_url, caption=text, node_id="eWKsWq0y8Xlm39S9JEf1g")
    except Exception:
        # Fallback на обычное сообщение при ошибке
        if keyboard is not None:
