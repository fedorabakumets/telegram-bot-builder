...
@dp.callback_query(lambda c: c.data == "FVMYdp6eP4H7c2yQXTakA" or c.data.startswith("FVMYdp6eP4H7c2yQXTakA_btn_"))
async def handle_callback_FVMYdp6eP4H7c2yQXTakA(callback_query: types.CallbackQuery):
    await callback_query.answer()
    user_id = callback_query.from_user.id
    # Сохраняем правильную переменную в базу данных
    await update_user_data_in_db(user_id, "возраст", callback_query.data)
    logging.info(f"Переменная возраст сохранена: " + str(callback_query.data) + f" (пользователь {user_id})")
    
    # Обрабатываем узел FVMYdp6eP4H7c2yQXTakA: FVMYdp6eP4H7c2yQXTakA
    text = "Сколько лет"
    ...
    button_display_text = "Кнопка FVMYdp6eP4H7c2yQXTakA"
    ...
    next_node_id = "FVMYdp6eP4H7c2yQXTakA"
    try:
        ...
        if next_node_id == "start":
        ...
        elif next_node_id == "FVMYdp6eP4H7c2yQXTakA":
            nav_text = "Сколько лет"
            ...
            user_data[user_id]["waiting_for_input"] = {
                ...
                "node_id": "FVMYdp6eP4H7c2yQXTakA",
                "next_node_id": "NOteoCOScfz_vFo9xCOD1",
                "min_length": 0,
                "max_length": 0,
                "retry_message": "Пожалуйста, попробуйте еще раз.",
                "success_message": "Спасибо за ваш ответ!"
            }
        elif next_node_id == "NOteoCOScfz_vFo9xCOD1":
    ...
    user_data[callback_query.from_user.id]["waiting_for_input"] = "FVMYdp6eP4H7c2yQXTakA"
    ...
    user_data[callback_query.from_user.id]["input_target_node_id"] = "NOteoCOScfz_vFo9xCOD1"
...
@dp.callback_query(lambda c: c.data == "NOteoCOScfz_vFo9xCOD1" or c.data.startswith("NOteoCOScfz_vFo9xCOD1_btn_"))
async def handle_callback_NOteoCOScfz_vFo9xCOD1(callback_query: types.CallbackQuery):
    await callback_query.answer()
    user_id = callback_query.from_user.id
    # Обрабатываем узел NOteoCOScfz_vFo9xCOD1: NOteoCOScfz_vFo9xCOD1
    ...
    # Ищем текст кнопки по callback_data
    button_display_text = "Кнопка NOteoCOScfz_vFo9xCOD1"
...
async def handle_user_input(message: types.Message):
            ...
            try:
                ...
                if next_node_id == "profile_command":
                    ...
                    if next_node_id == "start":
                        ...
                        await handle_callback_FVMYdp6eP4H7c2yQXTakA(fake_callback)
                    elif next_node_id == "NOteoCOScfz_vFo9xCOD1":
                        await handle_callback_NOteoCOScfz_vFo9xCOD1(fake_callback)
                    else:
                        logging.warning(f"Неизвестный следующий узел: {next_node_id}")
            except Exception as e:
                logging.error(f"Ошибка при переходе к следующему узлу {next_node_id}: {e}")
        ...
        if selected_option:
            ...
            if option_action == "url" and option_url:
                ...
                try:
                    ...
                    elif target_node_id == "FVMYdp6eP4H7c2yQXTakA":
                        await handle_callback_FVMYdp6eP4H7c2yQXTakA(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=target_node_id, message=message))
                    elif target_node_id == "NOteoCOScfz_vFo9xCOD1":
                        await handle_callback_NOteoCOScfz_vFo9xCOD1(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=target_node_id, message=message))
                    else:
                ...
                if next_node_id:
                    try:
                        ...
                        if next_node_id == "start":
                            ...
                            await handle_callback_FVMYdp6eP4H7c2yQXTakA(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=next_node_id, message=message))
                        elif next_node_id == "NOteoCOScfz_vFo9xCOD1":
                            await handle_callback_NOteoCOScfz_vFo9xCOD1(types.CallbackQuery(id="reply_nav", from_user=message.from_user, chat_instance="", data=next_node_id, message=message))
                        else:
                            logging.warning(f"Неизвестный следующий узел: {next_node_id}")
                    except Exception as e:
                        logging.error(f"Ошибка при переходе к следующему узлу {next_node_id}: {e}")
    ...
    if user_id in user_data and "waiting_for_input" in user_data[user_id]:
        ...
        if waiting_node_id == "start":
            ...
            try:
                # Отправляем сообщение для узла FVMYdp6eP4H7c2yQXTakA
                text = "Сколько лет"
                # Настраиваем новое ожидание ввода для узла FVMYdp6eP4H7c2yQXTakA
                user_data[user_id]["waiting_for_input"] = {
                    "type": "text",
                    "variable": "возраст",
                    "save_to_database": True,
            ...
            return
        elif waiting_node_id == "FVMYdp6eP4H7c2yQXTakA":
            ...
            # Переходим к следующему узлу
            try:
                # Отправляем сообщение для узла NOteoCOScfz_vFo9xCOD1
                text = "Новое сообщение"
                await message.answer(text)
                logging.info("✅ Переход к следующему узлу выполнен успешно")
            except Exception as e:
