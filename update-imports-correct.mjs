...
function getRelativePath(fromFile, toImport) {
  ...
  // Если путь не содержит расширение, добавляем .ts
  if (!toImport.endsWith('.ts') && !toImport.endsWith('.js')) {
    // Проверяем, является ли это директорией или файлом
    const potentialTsFile = path.resolve(libDir, toImport + '.ts');
    const potentialJsFile = path.resolve(libDir, toImport + '.js');
    const potentialDirIndexTs = path.resolve(libDir, toImport, 'index.ts');
    const potentialDirIndexJs = path.resolve(libDir, toImport, 'index.js');
    
    if (fs.existsSync(potentialTsFile)) {
      toImport = toImport + '.ts';
    } else if (fs.existsSync(potentialJsFile)) {
      toImport = toImport + '.js';
    } else if (fs.existsSync(potentialDirIndexTs)) {
      toImport = path.join(toImport, 'index.ts');
    } else if (fs.existsSync(potentialDirIndexJs)) {
      toImport = path.join(toImport, 'index.js');
    }
  }
