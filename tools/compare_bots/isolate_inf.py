"""
@fileoverview Изоляция: только INFINITY в цепочке сравнения.
"""
...
INF_CALC_IDS = {"init_arr", "fmt_inf", "push_inf"}
...
def main() -> None:
    ...
    nodes["bot-setv-init"]["data"]["autoTransitionTo"] = "bot-ub-inf-start"
    nodes["bot-setv-parse-inf"]["data"]["autoTransitionTo"] = "bot-setv-calc"
    nodes["bot-setv-parse-inf"]["data"]["enableAutoTransition"] = True

    calc = nodes["bot-setv-calc"]
    calc["data"]["assignments"] = [
        a for a in calc["data"]["assignments"] if a.get("id") in INF_CALC_IDS
    ]
    ...
    print("OK: init -> inf-start, parse-inf -> calc")
