"""
@fileoverview Изоляция: только CrazyBTC в цепочке сравнения.
"""
...
CRAZY_CALC_IDS = {"init_arr", "fmt_crazy", "push_crazy"}
...
def main() -> None:
    ...
    nodes["bot-setv-init"]["data"]["autoTransitionTo"] = "bot-ub-crazy-start"
    nodes["bot-setv-parse-crazy"]["data"]["autoTransitionTo"] = "bot-setv-calc"
    nodes["bot-setv-parse-crazy"]["data"]["enableAutoTransition"] = True

    calc = nodes["bot-setv-calc"]
    calc["data"]["assignments"] = [
        a for a in calc["data"]["assignments"] if a.get("id") in CRAZY_CALC_IDS
    ]
    ...
    print("OK: init -> crazy-start, parse-crazy -> calc")
