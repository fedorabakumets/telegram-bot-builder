 ...
 * @param botProcess - Процесс бота
 * @param launchId - Идентификатор запуска для привязки логов (опционально)
 * @returns Функция очистки — удаляет все навешенные слушатели
 */
export function setupProcessOutputListener(
  processKey: string,
  botProcess: any,
  launchId?: number
): () => void {
  ...
  const onStdout = (data: Buffer) => {
    ...
    sendOutputToTerminals(content, 'stdout', projectId, tokenId, launchId);
  };
  ...
  const onExit = async (code: number, signal: string) => {
    ...
    sendOutputToTerminals(content, 'status', projectId, tokenId, launchId);
