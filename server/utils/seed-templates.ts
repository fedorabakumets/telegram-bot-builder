...
/**
 * Инициализирует системные сценарии ботов в базе данных
 *
 * @param {boolean} [force=false] - Флаг, указывающий, нужно ли принудительно обновить сценарии
 *
 * @description
 * Функция проверяет наличие системных сценариев (созданных автором "Система") в базе данных.
 * Если сценарии уже существуют и флаг force не установлен, функция завершает работу.
 * Если флаг force установлен в true, функция удаляет все существующие системные сценарии
 * и создает новые стандартные сценарии для демонстрации возможностей системы.
 *
 * Функция создает сценарий "VProgulke Bot" - продвинутый бот знакомств для Санкт-Петербурга
 ...
 * @example
 * ```typescript
 * // Инициализировать сценарии, если они не существуют
 * await seedDefaultTemplates();
 *
 * // Принудительно обновить все системные сценарии
...
async function seedDefaultTemplates(force = false) {
  try {
    ...
    // Проверяем, есть ли уже системные сценарии
    const systemTemplates = existingTemplates.filter((t: BotTemplate) => t.authorName === 'Система');
    console.log(`📊 Найдено ${systemTemplates.length} системных сценариев`);

    if (force) {
      console.log('🔄 Принудительное обновление системных сценариев...');
      // Удаляем существующие системные сценарии
      for (const template of systemTemplates) {
        await storage.deleteBotTemplate(template.id);
      }
      console.log(`🗑️ Удалено ${systemTemplates.length} старых системных сценариев`);
    } else if (systemTemplates.length >= 1) {
      console.log('Системные сценарии уже существуют, пропускаем инициализацию');
      return;
    }



    // Создаем объединенный сценарий: ВПрогулке + Админ панель (ПОЛНАЯ ВЕРСИЯ)
    await storage.createBotTemplate({
      name: "🌟👮‍♂️ ВПрогулке + Админ панель - Полная версия",
      description: "Полноценный объединенный сценарий: продвинутый бот знакомств для Санкт-Петербурга со всеми функциями (метро, интересы, ориентация) + полный набор инструментов модерации группы",
      category: "community",
    ...
    console.log('✅ Объединенный сценарий ВПрогулке + Админ панель создан');

    // Создаем сценарий "Котик" - простая анкета знакомств (обновленная версия v3.3)
    await storage.createBotTemplate({
    ...
    console.log('✅ Сценарий Котик v3.3 создан (обновлен из приложенного файла)');

  } catch (error) {
    console.error('Ошибка при создании сценариев:', error);
  }
}
