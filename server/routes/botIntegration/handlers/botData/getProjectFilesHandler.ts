 ...
 * Для загруженных файлов (media_files) — удаляет запись из БД И физический файл из /uploads/.
 * Для входящих/исходящих (bot_messages) — удаляет только запись сообщения из БД.
 *
...
export async function deleteProjectFilesHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (source === "uploaded") {
      // Получаем записи для удаления физических файлов
      const rows = await db
      ...
      if (rows.length > 0) {
        ...
        for (const row of rows) {
          if (row.filePath) {
            const fullPath = join(process.cwd(), row.filePath);
            try {
      ...
      // incoming / outgoing — удаляем записи из bot_messages
      const result = await db
        .delete(botMessages)
        .where(and(eq(botMessages.projectId, projectId), inArray(botMessages.id, ids)));
      // drizzle не всегда возвращает rowCount, считаем по ids
      deleted = ids.length;
    }
