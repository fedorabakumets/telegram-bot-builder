...
## Команды бота (`command_trigger`)

Текст команды — **только латиница** (a-z, цифры, `_`), как в Telegram Bot API / `setMyCommands`: `/buy`, `/refund`, `/my_shop`. Кириллица в команде запрещена (`/купить` нельзя). Описание (`description`) и тексты сообщений — по-русски. Для MCP/ИИ: `docs/bot-json-prompt.md`, `docs/mcp/bot-builder.md`.
