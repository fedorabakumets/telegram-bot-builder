...
| **💬 Сообщение** | Отправляет текст, фото, видео, аудио или документы пользователю |
| **📋 Команда** | Обработка команд вроде `/start`, `/help` |
| **🗂️ Форма сбора данных** | Собирает информацию от пользователя (имя, почту и т.д.) |
| **🎤 Голос, Анимация, Геолокация** | Специальные типы сообщений |

**Логика переходов** - настраивается в свойствах каждого блока:
- **Кнопки** - пользователь выбирает один из вариантов
- **Автопереход** - переход к следующему блоку автоматически
- **Условные переходы** - "если выбрал это, то..." (указывается в свойствах)
