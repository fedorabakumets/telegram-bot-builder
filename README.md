...
sequenceDiagram
    ...
    Note over U,T: Создание бота
    U->>F: Перетаскивает блоки в редакторе
    F->>A: POST /api/projects/{id}/nodes
    A->>D: Сохраняет схему бота
    D-->>A: Подтверждение
    A-->>F: Обновленная схема
    
    Note over U,T: Генерация кода
    U->>F: Нажимает "Сгенерировать код"
    F->>A: POST /api/bots/{id}/generate
    A->>G: Запускает генератор
    G->>G: Создает Python код
    G->>D: Сохраняет код бота
    G-->>A: Готовый код
    A-->>F: Ссылка на скачивание
    
    Note over U,T: Деплой бота
    U->>F: Нажимает "Запустить бота"
    F->>A: POST /api/bots/{id}/deploy
    A->>G: Деплой на Railway/Vercel
    G->>T: Регистрирует webhook
    T-->>G: Подтверждение
    G-->>A: Статус деплоя
    A-->>F: Бот запущен
    
    Note over U,T: Работа бота
    U->>T: Пишет боту в Telegram
    T->>G: Webhook с сообщением
    G->>D: Обновляет статистику
    G->>T: Отправляет ответ
...
<details>
<summary><h2>✨ Что это может делать?</h2></summary>

<details>
<summary><h2>✨ Что это может делать? (нажми чтобы раскрыть)</h2></summary>
...
<details>
<summary><h2>🚀 Быстрый старт (5 минут)</h2></summary>
...
<details>
<summary><h2>☁️ Деплой в облако</h2></summary>
...
<details>
<summary><h2>🎓 Как создать своего первого бота? (пошаговое руководство)</h2></summary>
...
<details>
<summary><h2>🧩 Типы блоков (элементы для построения)</h2></summary>
...
| **� Стиокер** | Отправляет анимированный стикер |
| **🎤 Голосовое сообщение** | Отправляет голосовое сообщение |
| **� Геолоктация** | Отправляет карту с координатами |
...
<details>
<summary><h2>📚 Пример: Простой бот для пиццерии</h2></summary>
...
<details>
<summary><h2>🎯 Для кого это подходит?</h2></summary>
...
<details>
<summary><h2>📊 Статистика проекта</h2></summary>
...
<details>
<summary><h2>🛠️ Технические детали (для разработчиков)</h2></summary>
