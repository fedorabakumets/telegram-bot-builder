"""Fix: use {=thousands(user_amount)} in edit-ask-amount-loading nodes."""
import json
...
for sheet in data["sheets"]:
    for node in sheet.get("nodes", []):
        if node["id"].startswith("edit-ask-amount"):
            ...
            if "{user_amount}" in old:
                node["data"]["editMessageText"] = old.replace("{user_amount}", "{=thousands(user_amount)}")
                count += 1
                print(f"Fixed: {node['id']}")
                print(f"  New text: {node['data']['editMessageText'][:80]}...")
