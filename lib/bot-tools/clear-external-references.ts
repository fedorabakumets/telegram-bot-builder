/**
 * @fileoverview Чистка «висячих» ссылок на ноды, оказавшиеся вне допустимого набора
 * @description Lib-порт client/utils/sheets/clear-external-references.ts для
 * live-редактирования через БД. При переносе листа между проектами связи,
 * пересекающие границу листа (ссылки на ноды других листов исходного проекта),
 * должны обрываться. Функция возвращает НОВЫЙ объект data (deep copy), в котором
 * все ссылки на ноды вне набора validIds очищены. Покрывает простые поля-ссылки,
 * массивы с полем target (buttons, conditionalMessages c вложенными buttons,
 * commands, options, branches, parallelBranches) и вложенный inputConfig.next_node_id.
 * @module lib/bot-tools/clear-external-references
 */

/**
 * Простые строковые поля-ссылки на ноду (одна ссылка = одно поле).
 * Совпадают со списком в updateNodeReferencesInData (sheet-node-references.ts).
 */
 ...
 * @param validIds - Набор допустимых идентификаторов нод
...
/**
 * Возвращает глубокую копию data, в которой все ссылки на ноды вне validIds
 * очищены. Логика повторяет сброс полей при удалении одиночной связи: сброс
 * autoTransitionTo также выключает enableAutoTransition, а сброс
 * sourceMessageNodeId возвращает forward_message к текущему сообщению.
 * @param data - Объект данных ноды
...
export function clearExternalNodeReferences(data: any, validIds: Set<string>): any {
  ...
  // Простые строковые ссылки: удаляем поле, если цель вне набора
