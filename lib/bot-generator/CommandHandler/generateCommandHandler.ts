...
import { generateCommandHandlerInit } from './generateCommandHandlerInit';
...
export function generateCommandHandler(node: Node, userDatabaseEnabled: boolean, mediaVariablesMap?: Map<string, { type: string; variable: string; }>, allNodeIds: string[] = []): string {
  ...
  if (userDatabaseEnabled) {
    ...
    codeLines.push(`    user_name = await init_user_variables(user_id, message.from_user)`);
  ...
  if (node.data.enableConditionalMessages && node.data.conditionalMessages && node.data.conditionalMessages.length > 0) {
    ...
    // Добавляем инициализацию all_user_vars и переменных из БД
    const messageText = node.data?.messageText || node.data?.fallbackMessage || '';
    const cleanedText = stripHtmlTags(messageText);
    const initCode = generateCommandHandlerInit({ messageText: cleanedText, indent: '    ' });
    ...
    // Добавляем инициализацию all_user_vars и переменных из БД
    const initCode = generateCommandHandlerInit({ messageText: node.data?.messageText || '', indent: '    ' });
    codeLines.push(initCode);
  }
