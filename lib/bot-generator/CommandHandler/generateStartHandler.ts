...
import { generateStartHandlerInit } from './generateStartHandlerInit';
...
export function generateStartHandler(node: Node, userDatabaseEnabled: boolean, mediaVariablesMap?: Map<string, { type: string; variable: string; }>, allNodeIds: string[] = []): string {
  ...
  // Инициализируем all_user_vars и получаем переменные из БД
  const messageText = node.data?.messageText || '';
  const initCode = generateStartHandlerInit({ messageText, indent: '    ' });
