...
import {
  ...
  buildMediaSaveVarsParams,
  generateMediaSend,
  buildMediaSendParams,
  generateNoNodesAvailableWarning,
  generateParseMode,
  buildParseModeParams,
  ...
  if (nodes.length > 0) {
    nodes.forEach((targetNode, index) => {
      ...
      if (targetNode.type === 'message' && targetNode.data.keyboardType === "inline" && targetNode.data.buttons && targetNode.data.buttons.length > 0) {
        ...
        code += generateParseMode(buildParseModeParams(targetNode, '                '));
        code += generateMediaSaveVars(buildMediaSaveVarsParams(targetNode, '                '));
        code += generateAttachedMediaVars(buildAttachedMediaVarsParams(targetNode, '                '));
        code += generateMediaSend(buildMediaSendParams(targetNode, '                '));
