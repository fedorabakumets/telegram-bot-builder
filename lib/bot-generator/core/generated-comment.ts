...
export function addGeneratedComment(
  ...
  if (!commentsEnabled) return code;
 ...
 */
export function getSecurityCheckComment(
  checkType: 'private_chat' | 'admin' | 'auth',
  commentsEnabled: boolean = true
): string {
 ...
 */
export function getDataSavingComment(
  dataType: 'user_data' | 'statistics' | 'local_storage',
  commentsEnabled: boolean = true
): string {
  if (!commentsEnabled) return '';
  const comments = {
 ...
 */
export function getMessageProcessingComment(
  messageType: 'conditional' | 'fallback' | 'keyboard',
  commentsEnabled: boolean = true
): string {
  if (!commentsEnabled) return '';
  const comments = {
...
export function wrapWithGeneratedComments(
  ...
  if (!commentsEnabled) return codeLines;
...
export function createGeneratedBlock(
  ...
  return wrapWithGeneratedComments([code], generatorFile, blockDescription, commentsEnabled).join('\n');
}
