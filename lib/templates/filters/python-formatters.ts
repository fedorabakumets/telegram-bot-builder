 ...
 *
 * Для разрыва `'''` внутри тройных кавычек используется `''\\'` —
 * две одинарные кавычки + экранированная одинарная, что даёт `'''` в строке.
 *
 ...
 * formatTextForPython("Текст с '''\nстрока") // → `'''Текст с ''\\'\\nстрока'''`
 */
export function formatTextForPython(text: string): string {
  if (!text) return '""';
  if (text.includes('\n')) {
    // Многострочный режим: одинарные тройные кавычки
    // ''' → ''\\' — разрываем тройную кавычку: две одинарные + экранированная
    const escaped = text
      .replace(/\\/g, '\\\\')
      .replace(/'''/g, "''\\\'")  // ''' → ''\' (две одинарные + экранированная)
      .replace(/\r/g, '\\r');
    return `'''${escaped}'''`;
  }
