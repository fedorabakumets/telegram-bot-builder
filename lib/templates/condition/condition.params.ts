...
export interface ConditionBranchEntry {
  ...
  id: string;
  /** Оператор: "filled" | "empty" | "equals" | "contains" | "greater_than" | "less_than" | "between" | "is_private" | "is_group" | "is_channel" | "is_admin" | "is_premium" | "is_bot" | "is_subscribed" | "is_not_subscribed" | "else" */
  operator: 'filled' | 'empty' | 'equals' | 'contains' | 'greater_than' | 'less_than' | 'between' | 'is_private' | 'is_group' | 'is_channel' | 'is_admin' | 'is_premium' | 'is_bot' | 'is_subscribed' | 'is_not_subscribed' | 'else';
