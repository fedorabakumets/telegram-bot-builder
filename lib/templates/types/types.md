...
| `DynamicButtonsConfig`, `DynamicButtonsStyleMode` | `keyboard/dynamic-buttons.ts` | `keyboard/keyboard.py.jinja2` |
...
interface DynamicButtonsConfig {
  sourceVariable: string;     // Переменная с HTTP response
  arrayPath: string;          // Путь до массива внутри response
  textTemplate: string;       // Шаблон текста кнопки
  callbackTemplate: string;   // Шаблон callback_data
  styleMode: 'field' | 'template' | 'none';
  styleField: string;         // Поле style внутри элемента массива
  styleTemplate: string;      // Шаблон style для runtime
  columns: number;            // Количество колонок для inline keyboard
}
```
...
├── keyboard/dynamic-buttons.ts (DynamicButtonsConfig, DynamicButtonsStyleMode)
