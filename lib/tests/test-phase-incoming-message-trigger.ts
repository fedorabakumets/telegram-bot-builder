...
test('G01', 'пустой проект (нет incoming_message_trigger) → нет middleware в коде', () => {
  const p = makeCleanProject([makeMessageNode('msg1')]);
  const code = gen(p, 'g01');
  ok(!code.includes('incoming_message_trigger'), 'incoming_message_trigger НЕ должен быть в коде без триггеров');
});
...
test('H08', 'incoming_message_trigger → нет @dp.message() без аргументов (не fallback хендлер)', () => {
  const p = makeCleanProject([makeIncomingTriggerNode('imt1', 'msg1'), makeMessageNode('msg1')]);
  const code = gen(p, 'h08');
  // @dp.message() без аргументов — это fallback, его не должно быть
  ok(!code.includes('@dp.message()'), '@dp.message() без аргументов НЕ должен быть в коде');
});
...
test('J01', 'без incoming_message_trigger → нет `incoming_message_trigger` в коде', () => {
  const p = makeCleanProject([makeMessageNode('msg1')]);
...
test('J07', 'incoming_message_trigger → нет @dp.message() без аргументов', () => {
  const p = makeCleanProject([makeIncomingTriggerNode('imt1', 'msg1'), makeMessageNode('msg1')]);
