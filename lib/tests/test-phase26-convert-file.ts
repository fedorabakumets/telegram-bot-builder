 ...
 *  H. resolveMediaUrls — переменные {varName} в attachedMedia (6 тестов)
 *  I. convert-file — нормализация datetime через _normalize_row (5 тестов)
 *
 * Дополнительные блоки (J, K) — в test-phase26-convert-file-part2.ts
 */

import fs from 'fs';
import { execSync } from 'child_process';
import { generatePythonCode } from '../bot-generator.ts';
import { resolveMediaUrls } from '../templates/node-handlers/node-handlers.renderer.ts';
...
// ═══════════════════════════════════════════════════════════════════════════════
// БЛОК H: resolveMediaUrls — переменные {varName} в attachedMedia
// ═══════════════════════════════════════════════════════════════════════════════

console.log('\n── Блок H: resolveMediaUrls — переменные {varName} ─────────────');

test('H01', '{varName} в attachedMedia → attachedMediaUrls содержит переменную', () => {
  const result = resolveMediaUrls({ attachedMedia: ['{my_file}'] });
  ok(result.attachedMediaUrls.includes('{my_file}'),
    '{my_file} не найден в attachedMediaUrls');
});

test('H02', '{varName} в attachedMedia → imageUrl/videoUrl/audioUrl/documentUrl пустые', () => {
  const result = resolveMediaUrls({ attachedMedia: ['{export_file}'] });
  ok(result.imageUrl === '', `imageUrl должен быть пустым, получено: "${result.imageUrl}"`);
  ok(result.videoUrl === '', `videoUrl должен быть пустым, получено: "${result.videoUrl}"`);
  ok(result.audioUrl === '', `audioUrl должен быть пустым, получено: "${result.audioUrl}"`);
  ok(result.documentUrl === '', `documentUrl должен быть пустым, получено: "${result.documentUrl}"`);
});

test('H03', '{varName} в attachedMedia → все isLocal-флаги false', () => {
  const result = resolveMediaUrls({ attachedMedia: ['{my_file}'] });
  ok(!result.isLocalImageUrl, 'isLocalImageUrl должен быть false');
  ok(!result.isLocalVideoUrl, 'isLocalVideoUrl должен быть false');
  ok(!result.isLocalAudioUrl, 'isLocalAudioUrl должен быть false');
  ok(!result.isLocalDocumentUrl, 'isLocalDocumentUrl должен быть false');
});

test('H04', '{"__type":"file_id"...} НЕ считается переменной {varName}', () => {
  const fileIdJson = '{"__type":"file_id","fileIdsByToken":{"1":"abc123"},"mediaType":"photo"}';
  const result = resolveMediaUrls({ attachedMedia: [fileIdJson] });
  // file_id JSON не должен триггерить ветку hasVariableItems
  ok(result.attachedMediaUrls.includes(fileIdJson),
    'file_id JSON должен остаться в attachedMediaUrls');
  // imageUrl должен быть пустым (нет обычных URL), но ветка переменных не должна срабатывать
  ok(result.imageUrl === '', 'imageUrl должен быть пустым для file_id JSON');
});

test('H05', 'обычный http URL не считается переменной {varName}', () => {
  const result = resolveMediaUrls({ attachedMedia: ['https://example.com/photo.jpg'] });
  ok(result.imageUrl === 'https://example.com/photo.jpg',
    `imageUrl должен быть "https://example.com/photo.jpg", получено: "${result.imageUrl}"`);
  ok(result.attachedMediaUrls.includes('https://example.com/photo.jpg'),
    'URL не найден в attachedMediaUrls');
});

test('H06', 'несколько {varName} в attachedMedia → все попадают в attachedMediaUrls', () => {
  const result = resolveMediaUrls({ attachedMedia: ['{file_a}', '{file_b}'] });
  ok(result.attachedMediaUrls.includes('{file_a}'), '{file_a} не найден в attachedMediaUrls');
  ok(result.attachedMediaUrls.includes('{file_b}'), '{file_b} не найден в attachedMediaUrls');
  ok(result.imageUrl === '', 'imageUrl должен быть пустым при нескольких переменных');
});

// ═══════════════════════════════════════════════════════════════════════════════
// БЛОК I: convert-file — нормализация datetime через _normalize_row
// ═══════════════════════════════════════════════════════════════════════════════

console.log('\n── Блок I: convert-file — нормализация datetime ────────────────');

test('I01', '_normalize_row присутствует в теле обработчика convert_file', () => {
  const p = makeCleanProject([makeStartNode(), makeConvertFileNode('cf1')]);
  const code = gen(p, 'I01');
  const fnIdx = code.indexOf('async def handle_callback_cf1(');
  ok(fnIdx !== -1, 'handle_callback_cf1 не найден');
  const fnBody = code.slice(fnIdx, fnIdx + 4000);
  ok(fnBody.includes('_normalize_row'), '_normalize_row не найден в теле обработчика');
});

test('I02', 'def _normalize_row(row): присутствует в теле обработчика', () => {
  const p = makeCleanProject([makeStartNode(), makeConvertFileNode('cf1')]);
  const code = gen(p, 'I02');
  const fnIdx = code.indexOf('async def handle_callback_cf1(');
  ok(fnIdx !== -1, 'handle_callback_cf1 не найден');
  const fnBody = code.slice(fnIdx, fnIdx + 4000);
  ok(fnBody.includes('def _normalize_row(row):'),
    'def _normalize_row(row): не найден в теле обработчика');
});

test('I03', 'isoformat() присутствует в _normalize_row (конвертация datetime)', () => {
  const p = makeCleanProject([makeStartNode(), makeConvertFileNode('cf1')]);
  const code = gen(p, 'I03');
  const fnIdx = code.indexOf('async def handle_callback_cf1(');
  ok(fnIdx !== -1, 'handle_callback_cf1 не найден');
  const fnBody = code.slice(fnIdx, fnIdx + 4000);
  ok(fnBody.includes('isoformat()'), 'isoformat() не найден в теле обработчика');
});

test('I04', '_input_data = [_normalize_row(r) for r in _input_data] присутствует', () => {
  const p = makeCleanProject([makeStartNode(), makeConvertFileNode('cf1')]);
  const code = gen(p, 'I04');
  const fnIdx = code.indexOf('async def handle_callback_cf1(');
  ok(fnIdx !== -1, 'handle_callback_cf1 не найден');
  const fnBody = code.slice(fnIdx, fnIdx + 4000);
  ok(fnBody.includes('_input_data = [_normalize_row(r) for r in _input_data]'),
    '_input_data = [_normalize_row(r) for r in _input_data] не найдено');
});

test('I05', 'Синтаксис OK с нормализацией datetime (csv + json)', () => {
  for (const fmt of ['csv', 'json'] as const) {
    const p = makeCleanProject([makeStartNode(), makeConvertFileNode('cf1', {
      convertFileFormat: fmt,
    })], true);
    const code = genDB(p, `I05_${fmt}`);
    syntax(code, `I05_${fmt}`);
  }
});
...
console.log(`╚══════════════════════════════════════════════════════════════╝\n`);
console.log('  → Блоки J (psql-query datetime) и K (message file-var) — в test-phase26-convert-file-part2.ts');
