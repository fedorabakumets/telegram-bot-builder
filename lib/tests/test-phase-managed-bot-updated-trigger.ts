...
test('A01', 'managed_bot_updated_trigger → генерирует @dp.message декоратор', () => {
  const p = makeCleanProject([makeManagedBotUpdatedTriggerNode('mbu1', 'msg1'), makeMessageNode('msg1')]);
  const code = gen(p, 'a01');
  ok(code.includes('@dp.message('), '@dp.message должен быть в коде');
  ok(code.includes('managed_bot_created'), 'managed_bot_created должен быть в коде');
  ok(!code.includes('@dp.update.outer_middleware()'), '@dp.update.outer_middleware() НЕ должен быть в коде');
});
...
test('A02', 'имя обработчика содержит nodeId', () => {
  ...
  ok(code.includes('managed_bot_updated_trigger_mbu_abc_handler'), 'имя обработчика должно содержать nodeId');
});
...
test('A08', 'без autoTransitionTo → обработчик не генерируется', () => {
  ...
  ok(!code.includes('managed_bot_updated_trigger_mbu_bad_handler'), 'обработчик НЕ должен генерироваться без autoTransitionTo');
});
...
test('A09', 'два триггера → два обработчика', () => {
  ...
  ok(code.includes('managed_bot_updated_trigger_mbu1_handler'), 'первый обработчик должен быть в коде');
  ok(code.includes('managed_bot_updated_trigger_mbu2_handler'), 'второй обработчик должен быть в коде');
});
...
test('E01', 'обработчик содержит FakeCallbackQuery', () => {
  const p = makeCleanProject([makeManagedBotUpdatedTriggerNode('mbu1', 'msg1'), makeMessageNode('msg1')]);
...
test('E04', 'FakeCallbackQuery содержит self.message', () => {
  const p = makeCleanProject([makeManagedBotUpdatedTriggerNode('mbu1', 'msg1'), makeMessageNode('msg1')]);
  const code = gen(p, 'e04');
  ok(code.includes('self.message'), 'self.message должен быть в FakeCallbackQuery');
});
