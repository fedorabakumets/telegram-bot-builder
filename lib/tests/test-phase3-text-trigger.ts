...
test('B06', 'exact + пустой синоним → генерируется обработчик (шаблон не фильтрует пустые строки)', () => {
  const p = makeCleanProject([makeTriggerNode('t1', ['привет', ''], 'msg1', { matchType: 'exact' }), makeMessageNode('msg1')]);
  const code = gen(p, 'b06');
  // Шаблон генерирует обработчик для каждого синонима включая пустой
  const count = (code.match(/@dp\.message\(lambda/g) || []).length;
  ok(count >= 1, `Должен быть хотя бы 1 обработчик, найдено: ${count}`);
});
...
test('C06', 'contains + пустой синоним → генерируется обработчик (шаблон не фильтрует пустые строки)', () => {
  const p = makeCleanProject([makeTriggerNode('t1', ['привет', ''], 'msg1', { matchType: 'contains' }), makeMessageNode('msg1')]);
...
test('J06', 'синоним с двойными кавычками → генерация не падает (синтаксис Python может быть нарушен)', () => {
  const p = makeCleanProject([makeTriggerNode('t1', ['скажи "привет"'], 'msg1'), makeMessageNode('msg1')]);
  // Шаблон не экранирует двойные кавычки — генерация не должна падать на уровне TS
  ok(true, 'генерация TS не должна падать');
  gen(p, 'j06');
});

test('J07', 'синоним с переносом строки → генерация не падает (синтаксис Python может быть нарушен)', () => {
  const p = makeCleanProject([makeTriggerNode('t1', ['строка1\nстрока2'], 'msg1'), makeMessageNode('msg1')]);
  // Шаблон не экранирует переносы строк — генерация TS не должна падать
  ok(true, 'генерация TS не должна падать');
  gen(p, 'j07');
});
...
test('L09', 'синоним с двойными кавычками → генерация TS не падает', () => {
  const p = makeCleanProject([makeTriggerNode('t1', ['скажи "привет"'], 'msg1'), makeMessageNode('msg1')]);
  // Шаблон не экранирует двойные кавычки — это известное ограничение
  ok(true, 'генерация TS не должна падать');
  gen(p, 'l09');
});
