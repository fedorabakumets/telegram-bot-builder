...
test('K01', 'exact + adminOnly + requiresAuth → все проверки', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', ['привет'], 'msg1', { matchType: 'exact', adminOnly: true, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
...
test('K02', 'contains + adminOnly + requiresAuth → синтаксис OK', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', ['привет'], 'msg1', { matchType: 'contains', adminOnly: true, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
  syntax(gen(p, 'k02'), 'k02');
});

test('K03', '3 синонима + adminOnly + requiresAuth → каждый обработчик имеет все проверки', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', ['привет', 'хай', 'hello'], 'msg1', { adminOnly: true, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
...
test('K06', 'exact + adminOnly → порядок: admin ДО mock_callback', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', ['привет'], 'msg1', { matchType: 'exact', adminOnly: true }),
    makeMessageNode('msg1'),
  ]);
  ...
  ok(adminIdx < mockIdx, 'проверка admin должна идти ДО создания mock_callback');
});
...
test('K08', 'adminOnly + requiresAuth → оба в коде', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', ['привет'], 'msg1', { adminOnly: true, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
...
test('K09', 'все флаги false → нет лишних проверок в обработчике', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', ['привет'], 'msg1', { adminOnly: false, requiresAuth: false }),
    makeMessageNode('msg1'),
  ]);
...
test('K10', 'синтаксис OK для всех комбинаций флагов', () => {
  const combos = [
    { adminOnly: false, requiresAuth: false },
    { adminOnly: true, requiresAuth: false },
    { adminOnly: false, requiresAuth: true },
    { adminOnly: true, requiresAuth: true },
  ];
