...
test('A01', 'command_trigger с autoTransitionTo → генерируется @dp.message(Command(...))', () => {
  // /start обрабатывается особо (CommandStart), поэтому для проверки общей генерации
  // обычного командного обработчика используется не-/start команда.
...
test('A02', 'слеш убирается из команды → Command("help") не Command("/help")', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/help', 'msg1'), makeMessageNode('msg1')]);
  const code = gen(p, 'a02');
  ok(code.includes('Command("help")'), 'Command("help") должен быть в коде');
  ok(!code.includes('Command("/help")'), 'Command("/help") НЕ должен быть в коде');
});
...
test('A06', 'содержит await handle_callback_<targetId>(mock_callback, state=state)', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/help', 'msg1'), makeMessageNode('msg1')]);
  const code = gen(p, 'a06');
  ok(code.includes('await handle_callback_msg1(mock_callback, state=state)'), 'await handle_callback_msg1(mock_callback, state=state) должен быть в коде');
});
...
test('C09', 'несколько триггеров, один adminOnly → только у него проверка', () => {
  // Используем не-/start команды, чтобы у каждого триггера был свой
  // command_trigger_<id>_handler с локальной проверкой прав.
  const p = makeCleanProject([
    makeTriggerNode('t1', '/admin', 'msg1', { adminOnly: true }),
    makeTriggerNode('t2', '/help', 'msg1', { adminOnly: false }),
    makeMessageNode('msg1'),
  ]);
  const code = gen(p, 'c09');
  // Извлекаем тело каждого обработчика и проверяем наличие is_admin только у t1
  const t1Idx = code.indexOf('async def command_trigger_t1_handler');
  const t1Body = code.substring(t1Idx, code.indexOf('\nasync def ', t1Idx + 1));
  const t2Idx = code.indexOf('async def command_trigger_t2_handler');
  const t2End = code.indexOf('\nasync def ', t2Idx + 1);
  const t2Body = t2End > 0 ? code.substring(t2Idx, t2End) : code.substring(t2Idx);
  ok(t1Body.includes('is_admin(user_id)'), 'is_admin должен быть в обработчике t1');
  ok(!t2Body.includes('is_admin(user_id)'), 'is_admin НЕ должен быть в обработчике t2');
});
...
test('D09', 'несколько триггеров, один requiresAuth → только у него проверка', () => {
  // Используем не-/start команды, чтобы у каждого триггера был свой обработчик.
  const p = makeCleanProject([
    makeTriggerNode('t1', '/secure', 'msg1', { requiresAuth: true }),
  ...
  const t2End = code.indexOf('\nasync def ', t2Idx + 1);
  const t2Body = t2End > 0 ? code.substring(t2Idx, t2End) : code.substring(t2Idx);
  ok(t1Body.includes('check_auth(user_id)'), 'check_auth должен быть в обработчике t1');
  ok(!t2Body.includes('check_auth(user_id)'), 'check_auth НЕ должен быть в обработчике t2');
});
...
test('F01', 'два триггера → два @dp.message(Command(...))', () => {
  // Не-/start команды, чтобы оба генерировались как обычные @dp.message(Command(...)).
  const p = makeCleanProject([
    makeTriggerNode('t1', '/menu', 'msg1'),
...
test('F04', 'два триггера с разными командами → оба в коде', () => {
  ...
  ok(code.includes('Command("menu")'), 'Command("menu") должен быть в коде');
  ok(code.includes('Command("help")'), 'Command("help") должен быть в коде');
});

test('F05', 'два триггера с одинаковой командой → оба генерируются (не дедуплицируются)', () => {
  // Используем не-/start команду, чтобы оба генерировались как обычные обработчики.
  const p = makeCleanProject([
    makeTriggerNode('t1', '/dup', 'msg1'),
    makeTriggerNode('t2', '/dup', 'msg1'),
    makeMessageNode('msg1'),
  ]);
...
test('F06', 'триггер /start + триггер /help → /start как CommandStart, /help как Command("help")', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1'),
    makeTriggerNode('t2', '/help', 'msg1'),
    makeMessageNode('msg1'),
  ...
  // /start обрабатывается особым образом через CommandStart, а не Command("start")
  ok(code.includes('CommandStart(deep_link='), 'CommandStart(deep_link=...) должен быть для /start');
...
test('F08', 'несколько триггеров → каждый вызывает свой handle_callback_<targetId>', () => {
  ...
  ok(code.includes('await handle_callback_msg1(mock_callback, state=state)'), 'handle_callback_msg1 должен быть');
  ok(code.includes('await handle_callback_msg2(mock_callback, state=state)'), 'handle_callback_msg2 должен быть');
});
...
test('G01', 'autoTransitionTo → message узел → handle_callback_<id> вызывается', () => {
  ...
  ok(code.includes('await handle_callback_msg1(mock_callback, state=state)'), 'handle_callback_msg1 должен вызываться');
});
...
test('H01', 'команда /start → обрабатывается особо через CommandStart (не Command("start"))', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1'), makeMessageNode('msg1')]);
  const code = gen(p, 'h01');
  ok(code.includes('@dp.message(CommandStart(deep_link=False))'), 'CommandStart(deep_link=False) должен быть для /start');
  ok(!code.includes('Command("start")'), 'Command("start") НЕ должен генерироваться для /start');
});
...
test('H04', 'команда без слеша "help" → обрабатывается корректно → Command("help")', () => {
  const p = makeCleanProject([makeTriggerNode('t1', 'help', 'msg1'), makeMessageNode('msg1')]);
...
test('J03', 'adminOnly: true + requiresAuth: false → только admin', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/admin', 'msg1', { adminOnly: true, requiresAuth: false }),
    makeMessageNode('msg1'),
  ]);
  const code = gen(p, 'j03');
  const handlerIdx = code.indexOf('async def command_trigger_t1_handler');
  const nextFnIdx = code.indexOf('\nasync def ', handlerIdx + 1);
  const handlerBody = nextFnIdx > 0 ? code.substring(handlerIdx, nextFnIdx) : code.substring(handlerIdx);
...
test('J04', 'adminOnly: false + requiresAuth: true → только auth', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/secure', 'msg1', { adminOnly: false, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
...
test('N04', 'один триггер → ровно один @dp.message(Command(...))', () => {
  // Не-/start команда: /start генерируется через CommandStart, а не Command(...).
  const p = makeCleanProject([makeTriggerNode('t1', '/help', 'msg1'), makeMessageNode('msg1')]);
...
test('N06', 'нет глобального MockCallback: каждое определение используется локально', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1'),
    makeTriggerNode('t2', '/help', 'msg1'),
    makeTriggerNode('t3', '/menu', 'msg1'),
  ...
  // Каждое определение class MockCallback: должно сопровождаться локальным
  // использованием mock_callback = MockCallback(...) внутри той же функции —
  // то есть класс не выносится в глобальную область и не переиспользуется.
  const mockCallbackDefs = (code.match(/class MockCallback:/g) || []).length;
  const mockCallbackUses = (code.match(/mock_callback = MockCallback\(/g) || []).length;
  ok(mockCallbackDefs === mockCallbackUses, `Каждое определение MockCallback (${mockCallbackDefs}) должно сопровождаться локальным использованием (${mockCallbackUses})`);
});
