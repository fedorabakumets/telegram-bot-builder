...
console.log('── Блок B: isPrivateOnly ─────────────────────────────────────────');

test('B01', 'isPrivateOnly: true → есть message.chat.type != \'private\'', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true }), makeMessageNode('msg1')]);
  const code = gen(p, 'b01');
  ok(code.includes("message.chat.type != 'private'"), "message.chat.type != 'private' должен быть в коде");
});

test('B02', 'isPrivateOnly: true → есть "Эта команда доступна только в приватных чатах"', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true }), makeMessageNode('msg1')]);
...
test('B03', 'isPrivateOnly: true → есть return после проверки', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true }), makeMessageNode('msg1')]);
...
test('B04', 'isPrivateOnly: false → нет message.chat.type != \'private\'', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: false }), makeMessageNode('msg1')]);
  const code = gen(p, 'b04');
  ok(!code.includes("message.chat.type != 'private'"), "message.chat.type != 'private' НЕ должен быть в коде");
});

test('B05', 'isPrivateOnly: false → нет проверки типа чата вообще', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: false }), makeMessageNode('msg1')]);
  const code = gen(p, 'b05');
  ok(!code.includes('chat.type'), 'chat.type НЕ должен быть в коде');
});

test('B06', 'isPrivateOnly: true → синтаксис OK', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true }), makeMessageNode('msg1')]);
  syntax(gen(p, 'b06'), 'b06');
});

test('B07', 'isPrivateOnly: true + adminOnly: true → обе проверки есть', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true, adminOnly: true }), makeMessageNode('msg1')]);
...
test('B08', 'isPrivateOnly: true + requiresAuth: true → обе проверки есть', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true, requiresAuth: true }), makeMessageNode('msg1')]);
  const code = gen(p, 'b08');
  ok(code.includes("message.chat.type != 'private'"), 'проверка приватного чата должна быть');
  ok(code.includes('check_auth(user_id)'), 'проверка auth должна быть');
});

test('B09', 'isPrivateOnly: true + adminOnly: true + requiresAuth: true → все три проверки', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true, adminOnly: true, requiresAuth: true }), makeMessageNode('msg1')]);
...
test('B10', 'isPrivateOnly: true → проверка идёт ДО adminOnly (порядок важен)', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true, adminOnly: true }), makeMessageNode('msg1')]);
...
test('C08', 'adminOnly: true + isPrivateOnly: false → только проверка admin', () => {
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { adminOnly: true, isPrivateOnly: false }), makeMessageNode('msg1')]);
...
test('F09', 'несколько триггеров с разными флагами → флаги независимы', () => {
  const p = makeCleanProject([
    ...
    makeTriggerNode('t3', '/menu', 'msg1', { isPrivateOnly: true }),
    makeMessageNode('msg1'),
  ]);
...
test('J01', 'все флаги false → только базовый обработчик без проверок', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: false, adminOnly: false, requiresAuth: false }),
    makeMessageNode('msg1'),
  ]);
...
test('J02', 'все флаги true → все три проверки в правильном порядке', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true, adminOnly: true, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
  ...
  const adminIdx = code.indexOf('is_admin(user_id)');
  const authIdx = code.indexOf('check_auth(user_id)');
  ok(privateIdx < adminIdx, 'private должна идти ДО admin');
  ok(adminIdx < authIdx, 'admin должна идти ДО auth');
});

test('J03', 'isPrivateOnly: true + adminOnly: false + requiresAuth: false → только private', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true, adminOnly: false, requiresAuth: false }),
    makeMessageNode('msg1'),
  ]);
  ...
  const nextFnIdx = code.indexOf('\nasync def ', handlerIdx + 1);
  const handlerBody = nextFnIdx > 0 ? code.substring(handlerIdx, nextFnIdx) : code.substring(handlerIdx);
  ok(handlerBody.includes("message.chat.type != 'private'"), 'проверка приватного чата должна быть');
  ok(!handlerBody.includes('is_admin('), 'проверка admin НЕ должна быть');
  ok(!handlerBody.includes('not await check_auth('), 'вызов check_auth НЕ должен быть в обработчике');
});

test('J04', 'isPrivateOnly: false + adminOnly: true + requiresAuth: false → только admin', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: false, adminOnly: true, requiresAuth: false }),
    makeMessageNode('msg1'),
  ]);
...
test('J05', 'isPrivateOnly: false + adminOnly: false + requiresAuth: true → только auth', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: false, adminOnly: false, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
...
test('J06', 'isPrivateOnly: true + adminOnly: true + requiresAuth: false → private + admin', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true, adminOnly: true, requiresAuth: false }),
    makeMessageNode('msg1'),
  ]);
...
test('J07', 'isPrivateOnly: true + adminOnly: false + requiresAuth: true → private + auth', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true, adminOnly: false, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
...
test('J08', 'isPrivateOnly: false + adminOnly: true + requiresAuth: true → admin + auth', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: false, adminOnly: true, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
...
test('J09', 'все флаги true → синтаксис OK', () => {
  const p = makeCleanProject([
    makeTriggerNode('t1', '/start', 'msg1', { isPrivateOnly: true, adminOnly: true, requiresAuth: true }),
    makeMessageNode('msg1'),
  ]);
...
test('J10', 'все флаги true + 5 триггеров → синтаксис OK', () => {
  const triggers = Array.from({ length: 5 }, (_, i) =>
    makeTriggerNode(`t${i}`, `/cmd${i}`, 'msg1', { isPrivateOnly: true, adminOnly: true, requiresAuth: true })
  );
  const p = makeCleanProject([...triggers, makeMessageNode('msg1')]);
...
test('L05', 'description с двойными кавычками → генерация не падает', () => {
  // Примечание: description с двойными кавычками может вызывать синтаксическую ошибку
  // в BotCommand секции main() — это известное ограничение генератора.
  // Тест проверяет только что генерация не бросает исключение TypeScript.
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { description: 'Команда "старт"' }), makeMessageNode('msg1')]);
  ok(true, 'генерация не должна бросать исключение');
  gen(p, 'l05');
});
...
test('L07', 'description с переносом строки → генерация не падает', () => {
  // Примечание: description с переносом строки может вызывать синтаксическую ошибку
  // в BotCommand секции main() — это известное ограничение генератора.
  // Тест проверяет только что генерация не бросает исключение TypeScript.
  const p = makeCleanProject([makeTriggerNode('t1', '/start', 'msg1', { description: 'Строка 1\nСтрока 2' }), makeMessageNode('msg1')]);
  ok(true, 'генерация не должна бросать исключение');
  gen(p, 'l07');
});
...
test('L09', 'command_trigger без поля description → генерация не падает', () => {
  const trigger = {
    ...
    data: { command: '/start', showInMenu: true, isPrivateOnly: false, adminOnly: false, requiresAuth: false, autoTransitionTo: 'msg1', buttons: [], keyboardType: 'none' },
  };
  const p = makeCleanProject([trigger, makeMessageNode('msg1')]);
  ok(true, 'генерация не должна падать');
  gen(p, 'l09');
...
test('L10', 'command_trigger без поля showInMenu → генерация не падает', () => {
  const trigger = {
    id: 't1', type: 'command_trigger', position: { x: 0, y: 0 },
    data: { command: '/start', description: 'Старт', isPrivateOnly: false, adminOnly: false, requiresAuth: false, autoTransitionTo: 'msg1', buttons: [], keyboardType: 'none' },
  };
  const p = makeCleanProject([trigger, makeMessageNode('msg1')]);
  ok(true, 'генерация не должна падать');
  gen(p, 'l10');
