...
// ════════════════════════════════════════════════════════════════════════════
// БЛОК K: Новые поля пользователя (language_code, is_premium, is_bot)
// ════════════════════════════════════════════════════════════════════════════

console.log('── Блок K: Новые поля пользователя ──────────────────────────────');

test('K01', 'start + DB → init_user_variables содержит language_code', () => {
  const code = genDB(clone(BASE), 'k01');
  ok(code.includes('language_code'), 'language_code должен быть в коде');
});

test('K02', 'start + DB → init_user_variables содержит is_premium', () => {
  const code = genDB(clone(BASE), 'k02');
  ok(code.includes('is_premium'), 'is_premium должен быть в коде');
});

test('K03', 'start + DB → init_user_variables содержит is_bot', () => {
  const code = genDB(clone(BASE), 'k03');
  ok(code.includes('is_bot'), 'is_bot должен быть в коде');
});

test('K04', 'start + NO DB → start_handler содержит language_code', () => {
  const code = gen(clone(BASE), 'k04');
  ok(code.includes('language_code'), 'language_code должен быть в start_handler без БД');
});

test('K05', 'start + NO DB → start_handler содержит is_premium', () => {
  const code = gen(clone(BASE), 'k05');
  ok(code.includes('is_premium'), 'is_premium должен быть в start_handler без БД');
});

test('K06', 'start + NO DB → start_handler содержит is_bot', () => {
  const code = gen(clone(BASE), 'k06');
  ok(code.includes('is_bot'), 'is_bot должен быть в start_handler без БД');
});

test('K07', 'start + DB → синтаксис OK с новыми полями', () => {
  const code = genDB(clone(BASE), 'k07');
  syntax(code, 'k07');
});

test('K08', 'start + NO DB → синтаксис OK с новыми полями', () => {
  const code = gen(clone(BASE), 'k08');
  syntax(code, 'k08');
});
