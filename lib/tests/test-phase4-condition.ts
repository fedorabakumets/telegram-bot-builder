...
test('A02', 'содержит get_user_variables(user_id).get("user_name")', () => {
  const p = makeCleanProject([makeConditionNode('cond1', 'user_name', [makeBranch('filled')])]);
  const code = gen(p, 'a02');
  ok(code.includes('get_user_variables(user_id).get("user_name"'), 'get_user_variables(user_id).get("user_name" должен быть в коде');
});
...
test('G01', 'variable = "user_name" → get_user_variables(user_id).get("user_name")', () => {
  const p = makeCleanProject([makeConditionNode('cond1', 'user_name', [makeBranch('filled')])]);
  const code = gen(p, 'g01');
  ok(code.includes('get_user_variables(user_id).get("user_name"'), 'get_user_variables(user_id).get("user_name" должен быть в коде');
});

test('G02', 'variable = "age" → get_user_variables(user_id).get("age")', () => {
  const p = makeCleanProject([makeConditionNode('cond1', 'age', [makeBranch('filled')])]);
  const code = gen(p, 'g02');
  ok(code.includes('get_user_variables(user_id).get("age"'), 'get_user_variables(user_id).get("age" должен быть в коде');
});
...
test('L03', 'тело функции содержит val = get_user_variables(', () => {
  const p = makeCleanProject([makeConditionNode('cond1', 'x', [makeBranch('filled')])]);
  const code = gen(p, 'l03');
  ok(code.includes('val = get_user_variables('), 'val = get_user_variables( должен быть в теле функции');
});
