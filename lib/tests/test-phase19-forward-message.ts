...
test('G02', 'несколько command_trigger могут вести в один forward_message без дублирования обработчика', () => {
  ...
  ok(countOccurrences(code, 'async def handle_callback_fwd1(') === 2, 'Для forward_message сейчас ожидаются два слоя обработчика');
});
...
test('J05', 'legacy autoTransition message -> forward_message подавляется, если forward_message уже привязан как source-link', () => {
  const code = gen(makeCleanProject([
    makeMessageNode('msg1', 'Шаг 1', { enableAutoTransition: true, autoTransitionTo: 'fwd1' }),
    makeForwardMessageNode('fwd1', { sourceMessageNodeId: 'msg1' }),
  ]), 'j05');
  const messageHandler = handlerBlock(code, 'msg1');
  ok(!messageHandler.includes('await handle_callback_fwd1(fake_callback)'), 'Legacy autoTransition не должен запускать forward_message при source-link');
  ok(!messageHandler.includes('⚡ Автопереход от узла msg1 к узлу fwd1'), 'В обработчике message не должно остаться автоперехода в forward_message');
...
test('L05', 'legacy autoTransition media -> forward_message подавляется, если forward_message уже привязан как source-link', () => {
  const code = gen(makeCleanProject([
    makeMediaNode('media1', ['https://ex.com/a.jpg'], { enableAutoTransition: true, autoTransitionTo: 'fwd1' }),
    makeForwardMessageNode('fwd1', { sourceMessageNodeId: 'media1' }),
  ]), 'l05');
  ok(!code.includes('await handle_callback_fwd1(fake_cb)'), 'Legacy autoTransition media -> forward_message не должен генерироваться при source-link');
...
test('Q04', 'несколько входящих связей не дублируют handle_callback_forward', () => {
  ...
  ok(countOccurrences(code, 'async def handle_callback_fwd1(') === 2, 'При нескольких входящих связях должен оставаться ожидаемый двойной слой обработчика');
});
