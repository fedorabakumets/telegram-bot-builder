...
test('A01', 'forward_message генерирует основной callback-обработчик', () => {
  ...
  ok(code.includes('async def handle_callback_fwd1(callback_query: types.CallbackQuery, state: FSMContext = None):'), 'Обработчик handle_callback_fwd1 должен быть в коде');
});
...
test('I01', 'start с inline-кнопкой на forward_message содержит callback_data целевого узла', () => {
  ...
  ok(code.includes('async def handle_callback_fwd1(callback_query: types.CallbackQuery, state: FSMContext = None):'), 'Обработчик forward_message должен быть сгенерирован');
});
...
test('J03', 'message с отдельной keyboard-нодой может вести в forward_message', () => {
  ...
  ok(code.includes('async def handle_callback_fwd1(callback_query: types.CallbackQuery, state: FSMContext = None):'), 'Обработчик forward_message должен присутствовать');
});
...
test('J06', 'message с video-источником сохраняет sent_message через _send_to_ для последующего lookup по sourceMessageNodeId', () => {
  const code = gen(makeCleanProject([
  ...
  // После рефакторинга отправка идёт через вложенную _send_to_msg_video,
  // которая использует bot.send_video вместо callback_query.message.answer_video
  ok(
    messageHandler.includes('async def _send_to_msg_video') || messageHandler.includes('bot.send_video'),
    'Message handler должен сохранять отправленное video-сообщение через _send_to_ или bot.send_video',
  );
  ok(
    messageHandler.includes('sent_message = await _send_to_msg_video') || messageHandler.includes('sent_message'),
    'sent_message должен быть присвоен результату отправки',
  );
});
...
test('N03', 'safe_name применяется к ID forward_message с дефисами', () => {
  ...
  ok(code.includes('async def handle_callback_fwd_node_1(callback_query: types.CallbackQuery, state: FSMContext = None):'), 'ID с дефисами должен санитизироваться в safe_name');
});
