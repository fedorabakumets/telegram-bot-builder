...
test('D04', 'URL + JSON file_id — оба присутствуют в коде И file_id не в ветке else', () => {
  const node = {
    id: 'msg1',
    type: 'message',
    position: { x: 0, y: 0 },
  ...
  // Оба должны быть в коде
  ok(code.includes('video.mp4'), 'URL видео должен быть в коде');
  ok(code.includes('json.loads'), 'json.loads должен быть в коде');
  ok(code.includes('BAACAgI_test'), 'file_id должен быть в коде');
  // file_id НЕ должен быть только в ветке else (т.е. должен быть доступен при наличии videoUrl)
  // Проверяем что json.loads идёт ПОСЛЕ send_video (не в отдельной else-ветке)
  const sendVideoIdx = code.indexOf('send_video');
  const jsonLoadsIdx = code.indexOf('json.loads');
  ok(sendVideoIdx !== -1, 'send_video должен быть в коде');
  ok(jsonLoadsIdx !== -1, 'json.loads должен быть в коде');
