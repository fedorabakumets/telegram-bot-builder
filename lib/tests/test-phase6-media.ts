 ...
 * Блок M: Расширенные типы файлов — 3gp, flv, oga, aac, pptx, 7z, rtf, json, xml, кириллица в имени
 */
...
console.log('\n╔══════════════════════════════════════════════════════════════╗');
console.log('║         Фаза 6 — Медиа (127 тестов)                        ║');
console.log('╚══════════════════════════════════════════════════════════════╝\n');
...
// ════════════════════════════════════════════════════════════════════════════
// БЛОК M: Расширенные типы файлов
// ════════════════════════════════════════════════════════════════════════════

console.log('── Блок M: Расширенные типы файлов ───────────────────────────────');

// ─── Фото (локальные) ────────────────────────────────────────────────────────

test('M01', 'start: attachedMedia=[/uploads/photo.jpg] → answer_photo (локальный jpg)', () => {
  const code = gen(patchStart({
...
test('M02', 'start: attachedMedia=[/uploads/photo.jpeg] → answer_photo (локальный jpeg)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/photo.jpeg'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm02');
  syntax(code, 'm02');
  ok(code.includes('answer_photo'), 'answer_photo должен быть');
...
test('M03', 'start: attachedMedia=[/uploads/photo.png] → answer_photo (локальный png)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/photo.png'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm03');
  syntax(code, 'm03');
  ok(code.includes('answer_photo'), 'answer_photo должен быть');
...
test('M04', 'start: attachedMedia=[/uploads/photo.webp] → answer_photo (локальный webp)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/photo.webp'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm04');
  syntax(code, 'm04');
  ok(code.includes('answer_photo'), 'answer_photo должен быть');
...
test('M05', 'start: attachedMedia=[/uploads/video.mp4] → answer_video (локальный mp4)', () => {
  const code = gen(patchStart({
...
test('M06', 'start: attachedMedia=[/uploads/video.mov] → answer_video (локальный mov)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/video.mov'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm06');
  syntax(code, 'm06');
  ok(code.includes('answer_video'), 'answer_video должен быть');
...
test('M07', 'start: attachedMedia=[/uploads/video.avi] → answer_video (локальный avi)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/video.avi'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm07');
  syntax(code, 'm07');
  ok(code.includes('answer_video'), 'answer_video должен быть');
...
test('M08', 'start: attachedMedia=[/uploads/video.mkv] → answer_video (локальный mkv)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/video.mkv'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm08');
  syntax(code, 'm08');
  ok(code.includes('answer_video'), 'answer_video должен быть');
...
test('M09', 'start: attachedMedia=[/uploads/video.3gp] → answer_video (локальный 3gp)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/video.3gp'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm09');
  syntax(code, 'm09');
  ok(code.includes('answer_video'), 'answer_video должен быть');
...
test('M10', 'start: attachedMedia=[/uploads/audio.mp3] → answer_audio (локальный mp3)', () => {
  const code = gen(patchStart({
...
test('M11', 'start: attachedMedia=[/uploads/audio.ogg] → answer_audio (локальный ogg)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/audio.ogg'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm11');
  syntax(code, 'm11');
  ok(code.includes('answer_audio'), 'answer_audio должен быть');
...
test('M12', 'start: attachedMedia=[/uploads/audio.wav] → answer_audio (локальный wav)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/audio.wav'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm12');
  syntax(code, 'm12');
  ok(code.includes('answer_audio'), 'answer_audio должен быть');
...
test('M13', 'start: attachedMedia=[/uploads/audio.m4a] → answer_audio (локальный m4a)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/audio.m4a'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm13');
  syntax(code, 'm13');
  ok(code.includes('answer_audio'), 'answer_audio должен быть');
...
test('M14', 'start: attachedMedia=[/uploads/audio.flac] → answer_audio (локальный flac)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/audio.flac'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm14');
  syntax(code, 'm14');
  ok(code.includes('answer_audio'), 'answer_audio должен быть');
...
test('M15', 'start: attachedMedia=[/uploads/audio.aac] → answer_audio (локальный aac)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/audio.aac'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm15');
  syntax(code, 'm15');
  ok(code.includes('answer_audio'), 'answer_audio должен быть');
...
test('M16', 'start: attachedMedia=[/uploads/doc.pdf] → answer_document (локальный pdf)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/doc.pdf'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm16');
  syntax(code, 'm16');
  ok(code.includes('answer_document'), 'answer_document должен быть');
...
test('M17', 'start: attachedMedia=[/uploads/doc.docx] → answer_document (локальный docx)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/doc.docx'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm17');
  syntax(code, 'm17');
  ok(code.includes('answer_document'), 'answer_document должен быть');
...
test('M18', 'start: attachedMedia=[/uploads/doc.xlsx] → answer_document (локальный xlsx)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/doc.xlsx'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm18');
  syntax(code, 'm18');
  ok(code.includes('answer_document'), 'answer_document должен быть');
...
test('M19', 'start: attachedMedia=[/uploads/doc.pptx] → answer_document (локальный pptx)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/doc.pptx'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm19');
  syntax(code, 'm19');
  ok(code.includes('answer_document'), 'answer_document должен быть');
...
test('M20', 'start: attachedMedia=[/uploads/doc.txt] → answer_document (локальный txt)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/doc.txt'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm20');
  syntax(code, 'm20');
  ok(code.includes('answer_document'), 'answer_document должен быть');
...
test('M21', 'start: attachedMedia=[/uploads/doc.zip] → answer_document (локальный zip)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/doc.zip'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm21');
  syntax(code, 'm21');
  ok(code.includes('answer_document'), 'answer_document должен быть');
...
test('M22', 'start: attachedMedia=[/uploads/doc.csv] → answer_document (локальный csv)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/doc.csv'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm22');
  syntax(code, 'm22');
  ok(code.includes('answer_document'), 'answer_document должен быть');
...
// ─── HTTP-ссылки на новые форматы ────────────────────────────────────────────

test('M23', 'start: attachedMedia=[https://example.com/video.3gp] → answer_video (http 3gp)', () => {
  const code = gen(patchStart({
    attachedMedia: ['https://example.com/video.3gp'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm23');
  syntax(code, 'm23');
  ok(code.includes('answer_video'), 'answer_video должен быть');
...
test('M24', 'start: attachedMedia=[https://example.com/audio.aac] → answer_audio (http aac)', () => {
  const code = gen(patchStart({
    attachedMedia: ['https://example.com/audio.aac'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm24');
  syntax(code, 'm24');
  ok(code.includes('answer_audio'), 'answer_audio должен быть');
...
test('M25', 'start: attachedMedia=[https://example.com/doc.pptx] → answer_document (http pptx)', () => {
  const code = gen(patchStart({
    attachedMedia: ['https://example.com/doc.pptx'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm25');
  syntax(code, 'm25');
  ok(code.includes('answer_document'), 'answer_document должен быть');
...
test('M26', 'start: attachedMedia=[/uploads/реальный_файл.jpg] (кириллица в имени) → answer_photo', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/реальный_файл.jpg'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm26');
  syntax(code, 'm26');
  ok(code.includes('answer_photo'), 'answer_photo должен быть');
...
test('M27', 'start: attachedMedia=[/uploads/84/2026-03-19/1773938366629-668137492-____.jpg] → answer_photo (реальный путь)', () => {
  const code = gen(patchStart({
    attachedMedia: ['/uploads/84/2026-03-19/1773938366629-668137492-____.jpg'],
    imageUrl: '', videoUrl: '', audioUrl: '', documentUrl: '',
  }), 'm27');
  syntax(code, 'm27');
  ok(code.includes('answer_photo'), 'answer_photo должен быть');
