...
test('A09', 'aiogram.types KeyboardButton присутствует при reply клавиатуре', () => {
  const p = deepClone(BASE_PROJECT);
  p.sheets[0].nodes[0].data.keyboardType = 'reply';
  const code = gen(p, 'A09');
  assert(code.includes('KeyboardButton'), 'KeyboardButton должен быть при reply клавиатуре');
});

test('A09b', 'aiogram.types KeyboardButton отсутствует без reply клавиатуры', () => {
  const p = deepClone(BASE_PROJECT);
  p.sheets[0].nodes[0].data.keyboardType = 'inline';
  p.sheets[0].nodes[0].data.buttons = [];
  // Проверка что KeyboardButton нет в импортах (но может быть в InlineKeyboardButton)
  const code = gen(p, 'A09b');
  // Ищем точное совпадение: KeyboardButton как отдельный импорт
  const importSection = code.split('\n').slice(0, 50).join('\n');
  // KeyboardButton не должен быть отдельным импортом (но может быть частью InlineKeyboardButton)
  const hasStandaloneKeyboardButton = /^\s*KeyboardButton\s*[,)]/m.test(importSection) || 
                                      /^\s*KeyboardButton,/m.test(importSection);
  assert(!hasStandaloneKeyboardButton, 'KeyboardButton не должен быть отдельным импортом без reply клавиатуры');
});
...
test('A11', 'aiogram.utils.keyboard ReplyKeyboardBuilder присутствует при reply клавиатуре', () => {
  ...
  assert(code.includes('ReplyKeyboardBuilder'), 'ReplyKeyboardBuilder должен быть при reply клавиатуре');
});

test('A11b', 'aiogram.utils.keyboard ReplyKeyboardBuilder отсутствует без reply клавиатуры', () => {
  const p = deepClone(BASE_PROJECT);
  p.sheets[0].nodes[0].data.keyboardType = 'inline';
  p.sheets[0].nodes[0].data.buttons = [];
  const code = gen(p, 'A11b');
  assert(!code.includes('ReplyKeyboardBuilder'), 'ReplyKeyboardBuilder не должен быть без reply клавиатуры');
});
