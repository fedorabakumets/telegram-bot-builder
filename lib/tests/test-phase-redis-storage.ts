...
test('B03', 'if _redis_connected and _redis_client is not None: условный блок присутствует', () => {
  ok(codeNoDb.includes('if _redis_connected and _redis_client is not None'), 'условный блок проверки доступности Redis не найден');
});
...
test('B07', '_lock_acquired инициализируется до try', () => {
  const mainIndex = codeNoDb.indexOf('async def main():');
  const lockIndex = codeNoDb.indexOf('_lock_acquired = False', mainIndex);
  const tryIndex = codeNoDb.indexOf('try:', mainIndex);
  ok(mainIndex !== -1, 'async def main(): не найден');
  ok(lockIndex !== -1, '_lock_acquired = False не найден');
  ok(tryIndex !== -1, 'try: не найден');
...
test('G03', 'публикация событий защищена проверкой if _redis_connected and _redis_client is not None', () => {
...
test('G04', 'публикация событий не блокирует запуск при ошибке (try/except)', () => {
  ...
  // Каждый блок if _redis_connected and _redis_client is not None с publish должен содержать try/except
  const segments = codeNoDb.split('if _redis_connected and _redis_client is not None');
