...
// ════════════════════════════════════════════════════════════════════════════
// БЛОК P: callbackParseTemplate — извлечение переменных из callback_data
// ════════════════════════════════════════════════════════════════════════════

console.log('── Блок P: callbackParseTemplate — извлечение переменных ─────────');

/**
 * Создаёт callback_trigger с callbackParseTemplate
 * @param id - ID узла
 * @param callbackData - Значение callback_data (prefix)
 * @param matchType - Режим совпадения
 * @param targetId - ID целевого узла
 * @param parseTemplate - Шаблон разбора
 * @param saveVariables - Маппинг переменных
 */
function makeCallbackTriggerWithParse(
  id: string,
  callbackData: string,
  matchType: 'exact' | 'startswith',
  targetId: string,
  parseTemplate: string,
  saveVariables: Array<{ templateVar: string; saveAs: string }>,
) {
  return {
    id,
    type: 'callback_trigger',
    ...
    data: {
      ...
      requiresAuth: false,
      autoTransitionTo: targetId,
      callbackParseTemplate: parseTemplate,
      callbackSaveVariables: saveVariables,
      buttons: [],
      keyboardType: 'none',
    },
  };
...
test('P01', 'callbackParseTemplate → генерирует import re', () => {
  const p = makeCleanProject([
    makeCallbackTriggerWithParse('t1', 'rate_', 'startswith', 'msg1', 'rate_{from_id}_{to_id}', [
  ...
  ok(code.includes('import re as _re_cb_'), 'import re as _re_cb_ должен быть в коде');
});

test('P02', 'callbackParseTemplate → генерирует regex с именованными группами', () => {
  const p = makeCleanProject([
    makeCallbackTriggerWithParse('t1', 'rate_', 'startswith', 'msg1', 'rate_{from_id}_{to_id}', [
      { templateVar: 'from_id', saveAs: 'selected_from_id' },
      { templateVar: 'to_id', saveAs: 'selected_to_id' },
  ...
  ok(code.includes('(?P<from_id>'), 'regex должен содержать (?P<from_id>');
  ok(code.includes('(?P<to_id>'), 'regex должен содержать (?P<to_id>');
});

test('P03', 'callbackParseTemplate → генерирует fullmatch', () => {
  const p = makeCleanProject([
    makeCallbackTriggerWithParse('t1', 'rate_', 'startswith', 'msg1', 'rate_{from_id}_{to_id}', [
      { templateVar: 'from_id', saveAs: 'selected_from_id' },
      { templateVar: 'to_id', saveAs: 'selected_to_id' },
  ...
  ok(code.includes('.fullmatch('), '.fullmatch( должен быть в коде');
});

test('P04', 'callbackParseTemplate → генерирует set_user_var для каждой переменной', () => {
  const p = makeCleanProject([
    makeCallbackTriggerWithParse('t1', 'rate_', 'startswith', 'msg1', 'rate_{from_id}_{to_id}', [
      { templateVar: 'from_id', saveAs: 'selected_from_id' },
      { templateVar: 'to_id', saveAs: 'selected_to_id' },
  ...
  ok(code.includes('set_user_var(user_id, "selected_from_id"'), 'set_user_var для selected_from_id должен быть');
  ok(code.includes('set_user_var(user_id, "selected_to_id"'), 'set_user_var для selected_to_id должен быть');
});

test('P05', 'callbackParseTemplate → генерирует user_data[user_id]["saveAs"]', () => {
  const p = makeCleanProject([
    makeCallbackTriggerWithParse('t1', 'rate_', 'startswith', 'msg1', 'rate_{from_id}_{to_id}', [
      { templateVar: 'from_id', saveAs: 'selected_from_id' },
      { templateVar: 'to_id', saveAs: 'selected_to_id' },
  ...
  ok(code.includes('user_data[user_id]["selected_from_id"]'), 'user_data для selected_from_id должен быть');
  ok(code.includes('user_data[user_id]["selected_to_id"]'), 'user_data для selected_to_id должен быть');
});

test('P06', 'без callbackParseTemplate → НЕТ import re в обработчике', () => {
  const p = makeCleanProject([makeCallbackTriggerNode('t1', 'confirm', 'exact', 'msg1'), makeMessageNode('msg1')]);
  const code = gen(p, 'p06');
  const handlerIdx = code.indexOf('callback_trigger_t1_handler');
  const nextHandlerIdx = code.indexOf('async def ', handlerIdx + 50);
  const handlerBody = nextHandlerIdx > 0 ? code.substring(handlerIdx, nextHandlerIdx) : code.substring(handlerIdx, handlerIdx + 2000);
  ok(!handlerBody.includes('import re as _re_cb_'), 'import re НЕ должен быть в обработчике без parseTemplate');
});

test('P07', 'callbackParseTemplate с одной переменной → работает', () => {
  const p = makeCleanProject([
    makeCallbackTriggerWithParse('t1', 'item_', 'startswith', 'msg1', 'item_{id}', [
      { templateVar: 'id', saveAs: 'selected_item_id' },
    ]),
    makeMessageNode('msg1'),
  ]);
  const code = gen(p, 'p07');
  ok(code.includes('(?P<id>'), 'regex должен содержать (?P<id>');
  ok(code.includes('set_user_var(user_id, "selected_item_id"'), 'set_user_var для selected_item_id должен быть');
});

test('P08', 'callbackParseTemplate → синтаксис Python OK', () => {
  const p = makeCleanProject([
    makeCallbackTriggerWithParse('t1', 'rate_', 'startswith', 'msg1', 'rate_{from_id}_{to_id}', [
      { templateVar: 'from_id', saveAs: 'selected_from_id' },
      { templateVar: 'to_id', saveAs: 'selected_to_id' },
    ]),
    makeMessageNode('msg1', 'Курс: {selected_from_id} → {selected_to_id}'),
  ]);
  syntax(gen(p, 'p08'), 'p08');
});

test('P09', 'callbackParseTemplate + adminOnly → синтаксис Python OK', () => {
  const node = makeCallbackTriggerWithParse('t1', 'admin_', 'startswith', 'msg1', 'admin_{action}_{target}', [
    { templateVar: 'action', saveAs: 'admin_action' },
    { templateVar: 'target', saveAs: 'admin_target' },
  ]);
  (node.data as any).adminOnly = true;
  const p = makeCleanProject([node, makeMessageNode('msg1')]);
  syntax(gen(p, 'p09'), 'p09');
});

test('P10', 'callbackParseTemplate с тремя переменными → все три извлекаются', () => {
  const p = makeCleanProject([
    makeCallbackTriggerWithParse('t1', 'op_', 'startswith', 'msg1', 'op_{action}_{user}_{amount}', [
      { templateVar: 'action', saveAs: 'op_action' },
      { templateVar: 'user', saveAs: 'op_user' },
      { templateVar: 'amount', saveAs: 'op_amount' },
    ]),
    makeMessageNode('msg1'),
  ]);
