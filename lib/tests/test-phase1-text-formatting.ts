 ...
 * 32.  Текст с тройными кавычками ''' — не ломает Python (одинарные тройные кавычки)
...
console.log('\n╔══════════════════════════════════════════════════════════════╗');
console.log('║       Фаза 1 — Текст и форматирование (52 тестов)           ║');
console.log('╚══════════════════════════════════════════════════════════════╝\n');
...
test('32', "Текст с тройными кавычками ''' — не ломает Python", () => {
  // Многострочный текст с тройными одинарными кавычками — патчим message-узел
  // (command_trigger использует другой шаблон без format_python_text)
  const code = gen(patchMessage({ messageText: "Текст с '''тройными''' кавычками\nвторая строка", formatMode: 'none' }), 't32');
  assertSyntax(code, 't32');
  // Должны использоваться одинарные тройные кавычки (новое поведение)
  assert(code.includes("'''"), "должны использоваться одинарные тройные кавычки");
  // Тройные одинарные кавычки внутри текста должны быть разорваны
  assert(!code.includes("'''тройными'''"), "тройные кавычки внутри текста должны быть экранированы");
});
...
// ── 37. Конфликт markdown:true + formatMode:"html" → formatMode побеждает ─────
test('37', 'Конфликт markdown:true + formatMode:"html" → formatMode побеждает', () => {
  const code = gen(patchMessage({ messageText: '<b>Текст</b>', markdown: true, formatMode: 'html' }), 't37');
  assertSyntax(code, 't37');
  assert(
    code.includes('parse_mode="HTML"') || code.includes('ParseMode.HTML'),
    'formatMode:html должен побеждать над markdown:true'
  );
