...
test('H02', 'С autoTransitionTo + узел существует → await handle_callback_next_node(callback_query, state=state)', () => {
  ...
  ok(code.includes('await handle_callback_next_node(callback_query, state=state)'),
    'await handle_callback_next_node(callback_query, state=state) не найдено');
});
...
test('J10', 'Финальный мегатест: POST + заголовки + тело + переменные + статус + автопереход + DB → синтаксис OK', () => {
  ...
  ok(code.includes('await handle_callback_check_order(callback_query, state=state)'),
...
// ═══════════════════════════════════════════════════════════════════════════════
// БЛОК M: Автосохранение скалярных полей ответа
// ═══════════════════════════════════════════════════════════════════════════════

console.log('\n── Блок M: Автосохранение скалярных полей ответа ───────────────');

test('M01', 'Ответ-словарь → генерируется цикл for _fk, _fv in _response_data.items()', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestResponseVariable: 'project_detail',
  })]);
  const code = gen(p, 'M01');
  ok(
    code.includes('for _fk, _fv in _response_data.items()'),
    'for _fk, _fv in _response_data.items() не найдено — автосохранение полей отсутствует',
  );
});

test('M02', 'Синтаксис Python OK для узла с автосохранением полей', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestResponseVariable: 'project_detail',
    httpRequestUrl: 'http://localhost:5000/api/bot/projects/{callback_data}?telegram_id={user_id}',
  })]);
  syntax(gen(p, 'M02'), 'M02');
});

test('M03', 'Автосохранение использует isinstance(_response_data, dict)', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1')]);
  const code = gen(p, 'M03');
  ok(
    code.includes('isinstance(_response_data, dict)'),
    'isinstance(_response_data, dict) не найдено — проверка типа ответа отсутствует',
  );
});

test('M04', 'Автосохранение вызывает set_user_var с ключом responseVariable + "." + _fk', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestResponseVariable: 'project_detail',
  })]);
  const code = gen(p, 'M04');
  ok(
    code.includes('"project_detail." + _fk'),
    '"project_detail." + _fk не найдено — автосохранение не использует имя переменной ответа',
  );
});
...
test('N01', 'Интерактивная пагинация: nextOffset вычисляется в коде', () => {
  ...
  ok(code.includes('nextOffset'), 'nextOffset не найден в коде');
  ok(code.includes('prevOffset'), 'prevOffset не найден в коде');
});

test('N02', 'Интерактивная пагинация: читает offset из переменной users_offset', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('fetch_users', {
    httpRequestResponseVariable: 'users_data',
    httpRequestEnablePagination: true,
    httpRequestPaginationMode: 'interactive',
  ...
  ok(code.includes('"users_offset"'), '"users_offset" не найден в коде');
});

test('N03', 'Интерактивная пагинация: сохраняет currentPage и totalPages', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('fetch_users', {
    httpRequestResponseVariable: 'users_data',
    httpRequestEnablePagination: true,
    httpRequestPaginationMode: 'interactive',
  ...
  ok(code.includes('currentPage'), 'currentPage не найден');
  ok(code.includes('totalPages'), 'totalPages не найден');
});

test('N04', 'fetch_all: генерирует цикл сбора всех страниц', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('fetch_all', {
    httpRequestResponseVariable: 'all_data',
    httpRequestEnablePagination: true,
    httpRequestPaginationMode: 'fetch_all',
    httpRequestPaginationTotalField: 'total',
    httpRequestPaginationItemsField: 'items',
    httpRequestPaginationLimit: 50,
    httpRequestPaginationMaxPages: 5,
  })]);
  const code = gen(p, 'N04');
  ok(code.includes('_all_items'), '_all_items не найден');
  ok(code.includes('_fa_offset'), '_fa_offset не найден');
});

test('N05', 'fetch_all: ограничивает количество страниц через paginationMaxPages', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('fetch_all', {
    httpRequestResponseVariable: 'all_data',
    httpRequestEnablePagination: true,
    httpRequestPaginationMode: 'fetch_all',
    httpRequestPaginationMaxPages: 7,
    httpRequestPaginationLimit: 20,
  })]);
  const code = gen(p, 'N05');
  ok(code.includes('_fa_max_pages'), '_fa_max_pages не найден');
  ok(code.includes('7'), 'значение maxPages=7 не найдено');
});

test('N06', 'Без пагинации: код пагинации не генерируется', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestEnablePagination: false,
  })]);
  const code = gen(p, 'N06');
  ok(!code.includes('_pag_total'), '_pag_total найден — пагинация не должна генерироваться');
  ok(!code.includes('_all_items'), '_all_items найден — пагинация не должна генерироваться');
});

test('N07', 'Синтаксис Python OK для интерактивной пагинации', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('fetch_users', {
    httpRequestResponseVariable: 'users_data',
    httpRequestEnablePagination: true,
    httpRequestPaginationMode: 'interactive',
...
test('N08', 'Синтаксис Python OK для fetch_all', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('fetch_all', {
    httpRequestResponseVariable: 'all_data',
    httpRequestEnablePagination: true,
    httpRequestPaginationMode: 'fetch_all',
    ...
    httpRequestPaginationItemsField: 'items',
    httpRequestPaginationLimit: 50,
    httpRequestPaginationMaxPages: 10,
  })]);
