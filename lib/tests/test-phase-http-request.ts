...
test('A06', 'for _k, _v in _all_vars.items() присутствует (подстановка переменных в URL)', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1')]);
  const code = gen(p, 'A06');
  ok(code.includes('for _k, _v in _all_vars.items()'),
    'for _k, _v in _all_vars.items() не найдено');
});
...
test('B06', 'GET метод → НЕТ _body_raw (тело не генерируется)', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestMethod: 'GET',
  ...
  ok(!fnBody.includes('_body_raw'), '_body_raw найдено для GET — тело не должно генерироваться');
});
...
test('D02', 'POST с телом → _json_mod.loads(_body_raw) присутствует', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestMethod: 'POST',
    httpRequestBody: '{"email": "test@example.com"}',
  })]);
  const code = gen(p, 'D02');
  ok(code.includes('_json_mod.loads(_body_raw)'),
    '_json_mod.loads(_body_raw) не найдено для POST с телом');
});

test('D03', 'POST с телом → подстановка переменных (_body_raw.replace()', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestMethod: 'POST',
    httpRequestBody: '{"user_id": "{user_id}", "name": "{user_name}"}',
  })]);
  const code = gen(p, 'D03');
  ok(code.includes('_body_raw.replace('), '_body_raw.replace( не найдено — подстановка переменных в теле отсутствует');
});
...
test('D05', 'POST без тела → _body = None присутствует, НЕТ _body_raw', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestMethod: 'POST',
    httpRequestBody: '',
  })]);
  ...
  ok(fnIdx !== -1, 'handle_callback_http1 не найден');
  const fnBody = code.slice(fnIdx, fnIdx + 2000);
  ok(!fnBody.includes('_body_raw'), '_body_raw найдено для POST без тела — не должно быть');
});

test('D06', 'GET с телом → тело игнорируется (НЕТ _body_raw)', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestMethod: 'GET',
  ...
  ok(fnIdx !== -1, 'handle_callback_http1 не найден');
  const fnBody = code.slice(fnIdx, fnIdx + 2000);
  ok(!fnBody.includes('_body_raw'), '_body_raw найдено для GET — тело должно игнорироваться');
});

test('D07', 'DELETE с телом → тело игнорируется (НЕТ _body_raw)', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
  ...
  ok(fnIdx !== -1, 'handle_callback_http1 не найден');
  const fnBody = code.slice(fnIdx, fnIdx + 2000);
  ok(!fnBody.includes('_body_raw'), '_body_raw найдено для DELETE — тело должно игнорироваться');
});

test('D08', 'Тело с переменной {user_id} → подстановка работает', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestMethod: 'POST',
    httpRequestBody: '{"telegram_id": "{user_id}"}',
  })]);
  const code = gen(p, 'D08');
  ok(code.includes('_body_raw.replace('), 'Подстановка переменной {user_id} в теле не найдена');
});
...
test('E04', 'С statusVariable http_status → set_user_var(user_id, "http_status", _status_code)', () => {
  const p = makeCleanProject([makeStartNode(), makeHttpRequestNode('http1', {
    httpRequestStatusVariable: 'http_status',
  })]);
  const code = gen(p, 'E04');
  ok(code.includes('set_user_var(user_id, "http_status", _status_code)'),
    'set_user_var(user_id, "http_status", _status_code) не найдено');
});
...
test('J10', 'Финальный мегатест: POST + заголовки + тело + переменные + статус + автопереход + DB → синтаксис OK', () => {
  ...
  ok(code.includes('set_user_var(user_id, "order_status", _status_code)'),
