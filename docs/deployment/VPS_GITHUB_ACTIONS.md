...
На маленьком VPS (≈3 GB RAM, LXC) `docker compose build` на самом сервере легко кладёт машину (load уходит в десятки, SSH отваливается).  
...
**`swapon` / swap на Njalla**  
VPS на LXC: swap из гостя обычно нельзя. Это ограничение хостинга, не приложения. Либо апгрейд RAM, либо не билдить на проде (уже сделано).
