...
**Команды:** `command_trigger` — только латиница (`/buy`, `/refund`); кириллица в команде запрещена. Описание и сообщения — по-русски. Подробнее: [[mcp/bot-builder#Правило для агента: команды только латиницей]].
