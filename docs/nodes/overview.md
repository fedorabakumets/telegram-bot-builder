...
Кнопки можно задать двумя способами:

- **Прямо в message-ноде** — поля `keyboardType` + `buttons` в data
- **Отдельная keyboard-нода** — позволяет переиспользовать клавиатуру и обновлять кнопки без нового сообщения (через editMessageReplyMarkup)
