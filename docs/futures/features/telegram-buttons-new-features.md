...
### Переменные `{callback_data}` и `{button_text}` в сообщениях

**Реализовано:** 4 апреля 2026

При использовании узла `callback_trigger` в тексте следующего сообщения можно использовать специальные переменные:

- `{callback_data}` — значение `callback_data` нажатой кнопки (например `confirm_order`)
- `{button_text}` — текст кнопки, которую нажал пользователь (например `Подтвердить заказ`)
...
Переменные доступны в дропдауне «+ Переменная» в редакторе текста сообщения, когда в проекте есть хотя бы один узел `callback_trigger`.
