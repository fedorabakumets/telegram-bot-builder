...
## 9. Сравнение с n8n Postgres Node

n8n имеет встроенную Postgres-ноду — одну из самых популярных в экосистеме.

### Операции в n8n

| Операция | Описание |
|----------|---------|
| `Execute Query` | Произвольный SQL-запрос |
| `Select` | Визуальный построитель выборки (без SQL) |
| `Insert` | Вставка строк |
| `Insert or Update` | Апсерт |
| `Update` | Обновление по условию |
| `Delete` | Удаление строк или таблицы |

### Ключевые отличия

| | n8n | Наш `psql_query` |
|---|---|---|
| Подключение | Отдельный credential (хост, порт, логин, пароль) | Встроен — `pool` из `.env` бота, настраивать не нужно |
| Режимы | Select / Insert / Update / Delete + Execute Query | Execute Query (произвольный SQL) |
| Параметры | `$1, $2, $3` prepared statements | `{переменные}` через `replace_variables_in_text` |
| Батчинг | Single / Independently / Transaction | Один запрос |
| Результат | Передаётся следующей ноде как items | Сохраняется в переменную пользователя |
| Контекст | Универсальный (любой workflow) | Telegram-бот (user_id, user_data всегда доступны) |

### Что стоит взять у n8n в будущем

1. **Prepared statements** — `$1, $2` вместо подстановки строк через `replace_variables_in_text`. Безопаснее при работе с пользовательскими данными:
   ```sql
   SELECT * FROM orders WHERE user_id = $1 AND status = $2
   ```
   Параметры: `{user_id}`, `active`

2. **Визуальный построитель `Select`** — для новичков удобнее чем писать SQL вручную. Второй режим ноды рядом с `Execute Query`.

3. **Transaction** — если нужно несколько запросов атомарно (например, INSERT + UPDATE в одной транзакции).

### Вывод

Для нашего случая (Telegram-бот, один пул, простые запросы) — `Execute Query` с `{переменными}` достаточно на первой итерации. Визуальный построитель и prepared statements — следующий этап.
