...
**Дополнение (2026-07-28):** `DELETE /api/projects/:projectId/tokens/:tokenId` переведён на `requireTokenOwnership` / `hasProjectAccess` (коллабораторы могут удалять). Legacy `DELETE /api/tokens/:id` удалён (канон — project-scoped). `GET /api/projects/:id/tokens` и `bot-status.instance` отдают маску/`omit` вместо сырого Telegram token. См. [[features/token-project-access-delete]].
