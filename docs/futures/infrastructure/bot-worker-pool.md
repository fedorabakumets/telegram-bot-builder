...
- **Бот останавливается** → отправляем `stop_bot` → **ждём** `bot_exited`/`bot_stopped` → только потом DB/WS `stopped`
- **Последний бот остановлен** → воркер пустой → убиваем воркер

`CancelledError` в `main()` бота **должен re-raise**, иначе воркер эмитит `bot_exited:…:running`.
При restore старты ботов идут со **stagger ~250ms**, а `init_redis_client` делает retry с backoff.
