...
| Вызов произвольных функций | Только whitelist | Только `round`, `abs`, `int`, `float`, `min`, `max`, `str`, `thousands` |
...
### Фаза 1: MVP (высокий приоритет) 🔴

| # | Задача | Сложность | Описание |
|---|--------|-----------|----------|
| 1 | Добавить regex `{=...}` в `replace_variables_in_text` | Низкая | 10 строк кода |
| 2 | Вызывать `_eval_expr` для содержимого | Низкая | Функция уже существует |
| 3 | Обработка ошибок (fallback на исходный текст) | Низкая | Уже реализовано в _eval_expr |
| 4 | Тесты: арифметика в messageText | Средняя | 5-10 тест-кейсов |
| 5 | Обновить `docs/bot-json-prompt.md` | Низкая | Описать синтаксис для ИИ |

**Оценка:** 2-3 часа работы. Минимальные изменения, максимальный эффект.
