...
**✅ Шаг 3 (control) готов.** `db_start_bot(project_id, token_id?)` и `db_stop_bot(project_id, token_id, confirm)` в `bot-runtime-db.ts` (POST на `/bot/start|stop`); `db_stop_bot` с обязательным confirm-gate (без `confirm:true` запрос не уходит). История запусков и live-обновление UI приходят АВТОМАТИЧЕСКИ — эндпоинты вызывают те же `startBot`/`stopBot`, что и кнопки UI (эмит `bot-started`/`bot-stopped` в общем коде → WebSocket → инвалидация статуса). Запуск асинхронный → после старта проверять `db_bot_status`. Попутно **починен баг статуса**: `getBotTokenStatusHandler` не видел воркерных ботов (показывал `stopped` для работающих) — добавлена проверка `workerManager.isBotRunning`. Проверено вживую на 266: stop с confirm → история закрыта + карточка «Остановлен»; start → история открыта + карточка «🟢 Активный» live без F5.
