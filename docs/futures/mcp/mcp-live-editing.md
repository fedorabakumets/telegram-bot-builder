...
- Диск и БД — два независимых источника. Синхронизация ручная
  файловый мониторинг по умолчанию выключен.
...
**✅ Шаг 3 (control) готов.** `db_start_bot(project_id, token_id?)`, `db_stop_bot(project_id, token_id, confirm)`, `db_restart_bot(project_id, token_id?)` и `db_restart_all_bots(project_id, confirm)` в `bot-runtime-db.ts`. confirm-gate на `db_stop_bot` и `db_restart_all_bots` (без `confirm:true` запрос не уходит). **Гранулярность по токенам:** start/stop/restart все работают per-token (мультитокенные проекты) — для restart доработан серверный `handleBotRestart` (принимает опц. `tokenId`, резолв через `getBotToken` + проверка `projectId` совпадения: свой → рестарт, чужой проект → 403, несуществующий → 404; без `tokenId` — обратносовместимое поведение текущий инстанс + дефолтный токен). История запусков и live-обновление UI приходят АВТОМАТИЧЕСКИ — эндпоинты вызывают те же `startBot`/`stopBot`, что и кнопки UI. Запуск асинхронный → после старта/рестарта проверять `db_bot_status`. Попутно **починен баг статуса**: `getBotTokenStatusHandler` не видел воркерных ботов (показывал `stopped` для работающих) — добавлена проверка `workerManager.isBotRunning`. Проверено вживую на 266: stop/start/restart/restart-all, история, карточка «🟢 Активный» live без F5, межпроектная защита 403.
...
4. **Deny-by-default (профессиональный ключевой пункт).** Вместо точечной заплатки одного middleware — глобальный `requireAuth` на весь `/api/` с явным allowlist публичных роутов (`/api/health`, `/api/auth/*`, `/api/webhook/*`, `/api/setup*`, `/api/config` — собрать реальный список из `routes.ts`). Любой эндпоинт защищён по умолчанию; публичное помечается явно. Это закрывает не одну известную дыру (гостевой доступ по id), а весь класс «забыли повесить auth». Разнести текущий `authMiddleware` на `identifyUser` (обогащает `req.user`) + `requireAuth` (блокирует) — как в эталоне.
...
- Где запускается MCP относительно сервера (один хост? сеть?) — ✅ **частично решено**: remote Streamable HTTP на `POST /mcp` в том же Express (см. [`docs/mcp/remote-http.md`](../../mcp/remote-http.md)); stdio остаётся для локальной разработки. Follow-up: OAuth 2.1 + PKCE, Redis sessions.
