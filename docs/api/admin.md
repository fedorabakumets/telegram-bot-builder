...
Upsert по секциям провайдеров. Пустой `clientSecret` / `botToken` не удаляет существующие значения. `botUsername` опционально — резолв через getMe при заданном token.
