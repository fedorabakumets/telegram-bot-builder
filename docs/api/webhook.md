...
| Код | Описание |
|-----|----------|
| 200 | Апдейт принят. Body **пустое** (не JSON). Статус HTTP копируется с aiohttp или принудительно 200 при ошибке прокси. |
| 400 | Некорректные `projectId` или `tokenId` в path (не число) |
| 503 | Глобальный `setupGuard` — приложение ещё не настроено. До завершения setup (через /admin) webhook с production не работает. |
