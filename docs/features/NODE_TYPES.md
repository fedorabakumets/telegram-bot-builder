...
Выставляет счёт в текущий чат (валюта всегда звёзды). Без привязанной клавиатуры кнопку «Оплатить» рисует Телеграм сам. Можно связать узел с **Клавиатурой** (`keyboardNodeId`, как у сообщения): первая кнопка всегда «Оплатить» (`action: pay`, нельзя убрать), ниже — «Отмена» и другие. Клавиатура только **inline**. Переход по `autoTransitionTo` — **после успешной оплаты**, не с кнопки «Оплатить».
...
| Клавиатура | Связь с узлом `keyboard` (`keyboardNodeId`); первая кнопка — `pay` |
| Следующий узел | `autoTransitionTo` + `enableAutoTransition: true` — после оплаты |

Если в проекте есть хотя бы один такой узел, бот отвечает на `pre_checkout_query` всегда «да» (лимит Телеграма — 10 секунд). Ссылка на счёт, подписка на 30 дней и возврат — отдельные этапы роадмапа.
...
Срабатывает, когда пришла оплата звёздами, а счёт **не из текущей сессии** бота (ссылка, старый счёт после рестарта, другой поток). Если payload есть в `_stars_payment_targets` текущего запуска — сработает выход у `send_invoice`, не этот триггер.

Один общий обработчик `successful_payment`: сначала счёт, иначе первый подходящий триггер. Порядок фильтров: `exact` → `starts_with` → `all`. `pre_checkout` по-прежнему только при наличии `send_invoice`.
