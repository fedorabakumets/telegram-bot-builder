...
Выставляет счёт в текущий чат (валюта всегда звёзды). Кнопку «Оплатить» рисует Телеграм. Переход по `autoTransitionTo` — **после успешной оплаты**, не сразу после отправки счёта.
