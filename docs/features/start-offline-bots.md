...
Кнопка **«Запустить офлайн»** в шапке проекта и MCP `db_start_offline_bots` поднимают только остановленных ботов текущего проекта. Уже **running** не трогаются (для них — «Перезапустить» / `db_restart_all_bots`).

## Поток

1. UI confirm / MCP `confirm: true` → `POST /api/projects/:id/bot/start-offline-all`
2. Сервер фильтрует токены с `status !== running`, последовательно вызывает `startBot`
