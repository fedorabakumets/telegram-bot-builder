"""
🧪 Тест кнопочных ответов - Telegram Bot
...
async def start_handler(message: types.Message):
    ...
    text = "🧪 Тестовый бот с кнопочными ответами"
    
    # Создаем inline клавиатуру с кнопками
    builder = InlineKeyboardBuilder()
    builder.add(InlineKeyboardButton(text="🎯 Начать тест", callback_data="question-buttons"))
...
@dp.callback_query(lambda c: c.data == "question-buttons")
async def handle_callback_question_buttons(callback_query: types.CallbackQuery):
    await callback_query.answer()
    # Удаляем старое сообщение
    await callback_query.message.delete()
    
    text = "🔘 Выберите ваш любимый цвет:"
    
    # Создаем кнопки для выбора ответа
    builder = InlineKeyboardBuilder()
    builder.add(InlineKeyboardButton(text="🔴 Красный", callback_data="response_question-buttons_0"))
    builder.add(InlineKeyboardButton(text="🟢 Зеленый", callback_data="response_question-buttons_1"))
    builder.add(InlineKeyboardButton(text="🔵 Синий", callback_data="response_question-buttons_2"))
    keyboard = builder.as_markup()
    ...
    user_data[callback_query.from_user.id]["button_response_config"] = {
        "node_id": "question-buttons",
        "variable": "favorite_color",
        "save_to_database": True,
        "success_message": "Отлично! Цвет сохранен.",
        "allow_multiple": False,
        "options": [
            {"index": 0, "text": "🔴 Красный", "value": "red"},
            {"index": 1, "text": "🟢 Зеленый", "value": "green"},
            {"index": 2, "text": "🔵 Синий", "value": "blue"},
        ],
        "selected": []
    }
...
@dp.callback_query(F.data == "response_question-buttons_0")
async def handle_response_question-buttons_0(callback_query: types.CallbackQuery):
...
@dp.callback_query(F.data == "response_question-buttons_1")
async def handle_response_question-buttons_1(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
...
@dp.callback_query(F.data == "response_question-buttons_2")
async def handle_response_question-buttons_2(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
...
@dp.callback_query(F.data == "response_question-multi_0")
async def handle_response_question-multi_0(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
...
@dp.callback_query(F.data == "response_question-multi_1")
async def handle_response_question-multi_1(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
...
@dp.callback_query(F.data == "response_question-multi_2")
async def handle_response_question-multi_2(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
...
@dp.callback_query(F.data == "response_question-multi_3")
async def handle_response_question-multi_3(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
